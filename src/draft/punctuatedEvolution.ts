...
export function runPunctuatedEvolution(input: {
  ...
  for (let index = 0; index < candidateCount; index += 1) {
    const descendants = evolveManagerPopulation(competitors, input.season, `${input.seed}:burst-candidate:${index}`, input.historical ?? [], {targetSlotIds: selectedIds, protectedCopies: false, programOpportunities: Object.fromEntries(selectedIds.map(managerId => {
      const opportunities = input.programOpportunities?.[managerId];
      return [managerId, {...(opportunities?.entrypoints ?? {}), decisions: opportunities?.decisions ?? []}];
    }))});
