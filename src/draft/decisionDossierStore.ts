...
export interface DecisionDossierSummary {
  ...
  managers: number;
  battleCoverage: {artifacts: number; fullTraceBattles: number; currentEraFullTraceBattles: number; currentEraSeasons: number; currentEraManagers: number; latestCurrentEraSeason: number | null; latestSeasonFullTraceBattles: number; thresholds: {fullTraceBattles: number; managers: number; latestSeasonFullTraceBattles: number}; keySummaryBattles: number; overlappingBattles: number; representedBattles: number; unrepresentedBattles: number};
...
export function buildDecisionDossiers(options: DecisionDossierBuildOptions): DecisionDossierSummary {
  ...
  try {
    ...
    const currentEraCoverage = db.prepare("SELECT COUNT(DISTINCT season) seasons, MAX(season) latest_season FROM sources WHERE domain='battle' AND evidence_compatibility='exact-compatible' AND formal_activation_allowed=1 AND records>0").get() as any;
    const currentEraManagers = Number((db.prepare("SELECT COUNT(DISTINCT actor) count FROM decisions WHERE domain='battle' AND evidence_compatibility='exact-compatible' AND formal_activation_allowed=1").get() as any)?.count ?? 0), latestCurrentEraSeason = Number.isInteger(Number(currentEraCoverage?.latest_season)) ? Number(currentEraCoverage.latest_season) : null;
    const latestSeasonFullTraceBattles = latestCurrentEraSeason === null ? 0 : Number((db.prepare("SELECT COUNT(*) count FROM sources WHERE domain='battle' AND season=? AND evidence_compatibility='exact-compatible' AND formal_activation_allowed=1 AND records>0").get(latestCurrentEraSeason) as any)?.count ?? 0), coverageThresholds = {fullTraceBattles: 24, managers: 12, latestSeasonFullTraceBattles: 24};
    ...
    const summary: DecisionDossierSummary = {
      ...
      battleCoverage: {artifacts: discovery.battleArtifacts, fullTraceBattles: fullTraceKeys.size, currentEraFullTraceBattles, currentEraSeasons: Number(currentEraCoverage?.seasons ?? 0), currentEraManagers, latestCurrentEraSeason, latestSeasonFullTraceBattles, thresholds: coverageThresholds, keySummaryBattles: keySummaryKeys.size, overlappingBattles, representedBattles, unrepresentedBattles: Math.max(0, discovery.battleArtifacts - representedBattles)},
      outcomeAuthority: "terminal-observational", anomalies, healthy: anomalies.length === 0,
      formalDecisionCoverageReady: currentEraFullTraceBattles >= coverageThresholds.fullTraceBattles && currentEraManagers >= coverageThresholds.managers && latestSeasonFullTraceBattles >= coverageThresholds.latestSeasonFullTraceBattles && Number(counts.current_full_trace ?? 0) > 0 && Number(counts.formal_current_full_trace ?? 0) === Number(counts.current_full_trace ?? 0),
