...
if (!args.includes("--force") && fs.existsSync(summaryPath)) { const prior = read<V12AuditSummary>(summaryPath); cached = prior.schemaVersion === 4 && prior.inputSignature === signature; summary = cached ? prior : auditV12Output(out); } else summary = auditV12Output(out);
