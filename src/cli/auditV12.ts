...
if (!args.includes("--force") && mode === "quick" && fs.existsSync(summaryPath)) {
  ...
  cached = prior.schemaVersion === 5 && prior.inputSignature === signatureResult.signature;
  summary = cached ? prior : auditV12Output(out, signatureResult.signature);
} else summary = auditV12Output(out, signatureResult.signature);
...
console.log(JSON.stringify({cached, mode, seasons: summary.completedSeasons, fatal: summary.fatalCount, warnings: summary.warningCount, signature: {files: signatureResult.files, bytes: signatureResult.bytes, hashedFiles: signatureResult.hashedFiles, hashedBytes: signatureResult.hashedBytes}, elapsedMs: Date.now() - started, metrics: summary.metrics, summary: summaryPath}, null, 2));
