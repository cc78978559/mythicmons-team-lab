...
import {buildFormalCanaryAdapterRegistry, buildFormalCanaryHandoff, verifyFormalCanaryAdapterRegistry, verifyFormalCanaryHandoff, type FormalCanaryAdapterRegistry, type FormalCanaryHandoff} from "../ai/formalCanaryControl";

const args = process.argv.slice(2), command = args[0] && !args[0].startsWith("--") ? args[0] : "status", formal = path.resolve(option("--formal-validation", "output/tooling/formal-validation-stage5-delivery-v2")), out = path.resolve(option("--out", "output/tooling/formal-canary-control")), file = path.join(out, "handoff.json"), registryFile = path.resolve(option("--adapter-registry", path.join(out, "adapter-registry.json")));
if (command === "plan") {
  const handoff = buildFormalCanaryHandoff(read(path.join(formal, "freeze.json")), read(path.join(formal, "summary.json")), {seasons: integer("--seasons", 2), applicationRate: number("--rate", .1), maximumApplicationsPerSeason: integer("--max-applications", 12), adapterRegistry: optionalRegistry(registryFile)});
  ...
  try { const handoff = read(file) as FormalCanaryHandoff; verifyFormalCanaryHandoff(handoff); const freeze = read(path.join(formal, "freeze.json")), summary = read(path.join(formal, "summary.json")); const fresh = buildFormalCanaryHandoff(freeze, summary, {seasons: handoff.control.seasons, applicationRate: handoff.control.applicationRate, maximumApplicationsPerSeason: handoff.control.maximumApplicationsPerSeason, adapterRegistry: optionalRegistry(registryFile)}); if (fresh.sha256 !== handoff.sha256) throw new Error("Canary handoff is stale relative to formal validation or adapter registry"); console.log(JSON.stringify({available: true, healthy: true, ...compact(handoff)}, null, 2)); } catch (error) { console.log(JSON.stringify({available: fs.existsSync(file), healthy: false, issues: [{severity: "error", code: "formal-canary-handoff", message: error instanceof Error ? error.message : String(error)}]}, null, 2)); process.exitCode = 2; }
} else if (command === "register-adapter") {
  if (!args.includes("--reviewed")) throw new Error("Adapter registration requires explicit --reviewed confirmation");
  const id = required("--id"), implementationFile = path.resolve(required("--implementation")), targets = required("--targets").split(",").map(value => value.trim()).filter(Boolean), existing = optionalRegistry(registryFile);
  const adapters = [...(existing?.adapters.filter(adapter => adapter.id !== id).map(adapter => ({id: adapter.id, implementationFile: adapter.implementationFile, targets: adapter.targets})) ?? []), {id, implementationFile, targets}], registry = buildFormalCanaryAdapterRegistry(adapters);
  fs.mkdirSync(path.dirname(registryFile), {recursive: true}); write(registryFile, registry); console.log(JSON.stringify({registered: id, implementationFile, targets, registry: registryFile, sha256: registry.sha256}, null, 2));
} else throw new Error("Usage: npm run formal-canary -- <plan|status|doctor|register-adapter> [options]");
...
function optionalRegistry(file: string): FormalCanaryAdapterRegistry | null { if (!fs.existsSync(file)) return null; const value = read(file) as FormalCanaryAdapterRegistry; verifyFormalCanaryAdapterRegistry(value); return value; }
