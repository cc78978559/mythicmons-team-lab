...
function buildAcademyStates(): AcademyState[] {
  ...
  const weights = records.map(record => 1 + academyAlumniPerformance(record.state, record.alumni) * .5), totalWeight = weights.reduce((sum, value) => sum + value, 0) || 1;
  return records.map((record, index) => { const control = academyControls.find(value => value.academyId === record.state.academyId); return evolveAcademyState(record.state, record.alumni, cycle, academyEvolutionPercent / 100, {grant: academyGrantPool * weights[index] / totalWeight, maximumSpend: academyMaximumSpend * (control?.spendingMultiplier ?? 1), performanceRevenueRate: academyPerformanceRevenue}); }).sort((a, b) => a.academyId.localeCompare(b.academyId));
}
