...
function writePromotionPackage(promoted: Array<ReturnType<typeof resultRow>>): void {
  const state = read<{managers: DevelopmentManager[]}>(path.join(out, "league", "dynasty-state.json"));
  const sourceStateBytes = fs.readFileSync(path.join(source, "dynasty-state.json"));
  const auditPath = path.join(source, "audit-summary.json");
  const auditInputSignature = fs.existsSync(auditPath) ? read<{inputSignature?: string}>(auditPath).inputSignature : undefined;
  const payload = {schemaVersion: 2, source: {majorLeague: source, developmentLeague: path.join(out, "league"), birthSeason, developmentSeasons: seasons, cycle, seed: sourceState.seed, completedSeason: sourceState.completedSeason, stateSha256: crypto.createHash("sha256").update(sourceStateBytes).digest("hex"), fingerprint: sourceState.fingerprint, registryHash: sourceState.registry?.hash, auditInputSignature}, candidates: promoted.map(entry => { const manager = state.managers.find(value => value.id === entry.slotId)!, entrant = entrants.find(value => value.slotId === entry.slotId)!; return {...entry, currentProfile: manager.currentProfile, lineageHistory: manager.lineageHistory, career: combinedCareer(manager, entrant)}; })};
