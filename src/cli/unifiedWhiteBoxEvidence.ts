...
function runExperiment(hypothesis: UnifiedEvidenceCase, replica: UnifiedEvidenceReplica): void {
  ...
  try {
    const command = replica.runner === "lineup" ? lineupCommand(replica, directory) : replica.runner === "battle" ? battleCommand(replica, directory) : replica.runner === "memory" ? memoryCommand(replica, directory) : replica.runner==="learning"?learningCommand(replica,directory):replica.runner==="program-evolution"?programEvolutionCommand(replica,directory):replica.runner==="evolution"?evolutionCommand(replica,directory):replica.runner==="acquisition"?acquisitionCommand(replica,directory): [path.join(root, "src", "cli", "counterfactualWhiteBox.ts"), "--source", replica.root, "--out", directory, "--case-index", String(replica.reviewIndex), "--followup-seasons", String(followupSeasons)];
    ...
    const retention = replica.runner === "battle" || replica.runner === "memory" ? undefined : replica.runner==="program-evolution"||replica.runner==="evolution"||replica.runner==="acquisition"?[path.join(directory,"experiment"),path.join(directory,"control")].map(branch=>compactWhiteBoxRun(branch)):[path.join(directory, "incumbent"), path.join(directory,replica.runner==="learning"?"candidate":"whitebox")].map(branch => compactWhiteBoxRun(branch));
