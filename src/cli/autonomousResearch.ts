...
async function cycle(): Promise<Record<string, unknown>> {
  ...
  try {
    const programFile = path.join(stage3, "programs.json.gz"), corpusFile = path.join(stage3, "corpus.json.gz"), inputs = currentInputFingerprints(), inputSignature = digest(inputs), manifestFile = path.join(out, "manifest.json"); let existingManifest = optional<Manifest>(manifestFile);
...
function buildPlan(round: number, programs: ManagerProgramV2[], states: AutonomousResearchManagerState[], frontier: FrontierArchive, inputSignature: string, technicalBlacklist = new Set<string>()): RoundPlan {
  ...
  for (const program of programs) {
    ...
    agendas.push(buildAutonomousResearchAgenda({program, state, round, feasibleCases: feasible, seed: inputSignature}));
  }
...
function doctor(verifySources: boolean): Record<string, unknown> {
  ...
  try {
    validateManifestEnvelope(manifest); validateInputBindings(manifest.inputs, currentInputFingerprints());
  ...
  for (const round of manifestRounds) {
    try {
      ...
      if (plan.schemaVersion !== 1 || plan.version !== AUTONOMOUS_RESEARCH_VERSION || plan.activationStatus !== "shadow-only" || plan.round !== round.round || plan.inputSignature !== manifest.inputSignature || results.schemaVersion !== 1 || results.version !== AUTONOMOUS_RESEARCH_VERSION || results.activationStatus !== "shadow-only" || results.round !== round.round || results.planSha256 !== round.planSha256 || plan.items.length !== states.length || results.results.length !== states.length || plan.unassignedManagers.length) throw new Error("round envelope mismatch");
      ...
      if (plan.agendas.length !== states.length || itemByManager.size !== states.length || resultByManager.size !== states.length) throw new Error("round population mismatch");
      for (const state of states) {
        ...
        if (!item || !result || !agenda || item.round !== round.round || item.question.id !== agenda.ranked.find(question => question.id === item.question.id)?.id) throw new Error(`missing manager binding: ${state.managerId}`);
...
function currentInputFingerprints(): Record<string, Fingerprint> { return {programs: fingerprint(path.join(stage3, "programs.json.gz")), corpus: fingerprint(path.join(stage3, "corpus.json.gz")), stage3Summary: fingerprint(path.join(stage3, "summary.json"))}; }
