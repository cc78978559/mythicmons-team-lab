...
function retainedRecords(directory:string):any[]{const stateRecords=loadDynastyState<any>(path.join(directory,"dynasty-state.json")).decisionRecords??[];if(stateRecords.length)return stateRecords;const career=path.join(directory,"career-decisions","decision-ledger.json");return fs.existsSync(career)?read<any>(career).records??[]:[];}
