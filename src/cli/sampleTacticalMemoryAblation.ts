...
const out=path.resolve(option("--out","output/tactical-memory-ablation")),minimumConfidence=numberOption("--minimum-confidence",.05,0,1),maximumConfidence=numberOption("--maximum-confidence",1,0,1),targetSamples=integerOption("--target-samples",30,3,10000),minimumSeeds=integerOption("--minimum-seeds",10,2,targetSamples),minimumDecisivePairs=integerOption("--minimum-decisive-pairs",10,2,targetSamples),minimumDecisiveSeeds=integerOption("--minimum-decisive-seeds",5,2,minimumSeeds),maximumOneSidedP=numberOption("--maximum-one-sided-p",.1,.001,.5),maximumSamples=integerOption("--max-samples",Math.max(90,targetSamples),targetSamples,10000),maximumOutputMb=integerOption("--max-output-mb",2048,10,102400),minimumFreeGb=numberOption("--min-free-gb",10,0,10000),maximumLaunches=integerOption("--max-launches",10,1,1000);
if(maximumConfidence<=minimumConfidence)throw new Error("--maximum-confidence must be greater than --minimum-confidence");
const config={inputs,minimumConfidence,maximumConfidence,targetSamples,minimumSeeds,minimumDecisivePairs,minimumDecisiveSeeds,maximumOneSidedP,maximumOutputMb,minimumFreeGb};
...
async function runCandidate(candidate:Candidate):Promise<void>{
  ...
  try{
    ...
    const learned=await runBattle({...common,outDir:path.join(directory,"learned")}),learnedTraces=read<AiDecisionTrace[]>(learned.decisionLogPath),sourceVerified=JSON.stringify(sourceTraces)===JSON.stringify(learnedTraces);
    ...
    const ablated=await runBattle({...common,outDir:path.join(directory,"ablated"),aiOpponentModels:models}),ablatedTraces=read<AiDecisionTrace[]>(ablated.decisionLogPath);
    const sample:TacticalMemoryAblationSample={seed:candidate.seed,caseId:candidate.id,playerId:candidate.playerId,confidence:candidate.confidence,sourceVerified,firstDivergenceOrdinal:firstDivergence(learnedTraces,ablatedTraces,candidate.playerId),learned:outcome(learned),ablated:outcome(ablated)};
...
function scanCandidates():Candidate[]{const values=new Map<string,Candidate>();for(const root of inputs)for(const file of findNamed(root,"replay-input.json")){const sourceGame=path.dirname(file);if(!hasEvidence(sourceGame,"ai-decisions.json"))continue;try{const capsule=loadBattleReplayCapsule(file);if(!eligible(capsule))continue;for(const playerId of ["p1","p2"] as const){const confidence=capsule.input.aiOpponentModels[playerId].confidence;if(confidence+1e-12<minimumConfidence||(maximumConfidence<1&&confidence>=maximumConfidence))continue;const seed=capsule.input.seed.join("-"),id=digest(`${capsule.sha256}:${playerId}`);values.set(id,{id,sourceGame,replaySha256:capsule.sha256,seed,playerId,confidence:round(confidence),season:seasonFromPath(path.relative(root,file).replaceAll("\\","/"))});}}catch{continue;}}return[...values.values()].sort((a,b)=>b.confidence-a.confidence||a.seed.localeCompare(b.seed)||a.playerId.localeCompare(b.playerId)||a.sourceGame.localeCompare(b.sourceGame));}
...
function withoutLegacySampleCap(value:Omit<Manifest["config"],"maximumConfidence">&{maximumConfidence?:number;maximumSamples?:number}):Manifest["config"]{const copy={...value,maximumConfidence:value.maximumConfidence??1};delete copy.maximumSamples;return copy;}
