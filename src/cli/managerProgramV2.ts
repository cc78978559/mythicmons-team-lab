...
function build(): Summary {
  ...
  try {
    ...
    const behaviors = programs.map(managerProgramV2Behavior), acceptedRules = programs.reduce((sum, program) => sum + program.rules.length, 0), rejected = programs.reduce((sum, program) => sum + program.history.filter(entry => !entry.accepted).length, 0);
...
function buildCorpus(model: PositionValueModel, dossierPolicy: string): CorpusArchive {
  ...
  for (const file of files) {
    const relative = rel(corpusRoot, file), endFile = path.join(path.dirname(file), "end.json"); if (!fs.existsSync(endFile)) continue;
    const bytes = fs.readFileSync(file), endBytes = fs.readFileSync(endFile), traces = JSON.parse(bytes.toString("utf8")) as AiDecisionTrace[], terminal = JSON.parse(endBytes.toString("utf8")) as {winner?: string};
    sourceFingerprints[relative] = digestBytes(Buffer.concat([bytes, endBytes]));
    if (traces.some(trace => /\b(?:terastallize|dynamax)\b/i.test(trace.selected))) { forbidden += 1; continue; }
    if (!traces.length || !["Team A", "Team B"].includes(String(terminal.winner))) continue; accepted += 1;
    ...
    for (let index = 0; index < turns.length; index += 1) for (const playerId of ["p1", "p2"] as const) {
      const [turn, pair] = turns[index], trace = pair[playerId], opponent = pair[playerId === "p1" ? "p2" : "p1"], current = predictPairedPositionValue(trace.positionSnapshot!, opponent.positionSnapshot!, model);
      let next = terminal.winner === (playerId === "p1" ? "Team A" : "Team B") ? 1 : 0;
      if (index + 1 < turns.length) { const nextPair = turns[index + 1][1], nextTrace = nextPair[playerId], nextOpponent = nextPair[playerId === "p1" ? "p2" : "p1"]; next = predictPairedPositionValue(nextTrace.positionSnapshot!, nextOpponent.positionSnapshot!, model); }
      const featureValues = pairedPositionFeatures(trace.positionSnapshot!, opponent.positionSnapshot!), features = Object.fromEntries(POSITION_FEATURES.map((name, featureIndex) => [name, featureValues[featureIndex]]));
      ...
      samples.push({id: digest([battleId, playerId, turn]).slice(0, 24), battleId, clusterId, action: battleActionFamily(trace.selected), features, localValueDelta: round(clamp(next - current, -1, 1)), authority: "local-value-observational"});
    }
  }
...
function doctor(verifySources: boolean): {available: boolean; healthy: boolean; summary?: Summary; verifiedSources: number; issues: Array<{severity: "error" | "warning"; code: string; message: string}>} {
  ...
  if (!summary || !corpus || !archive) return {available: false, healthy: false, verifiedSources: 0, issues: [{severity: "error", code: "missing-artifacts", message: "Manager-program V2 artifacts are missing"}]};
  ...
  if (verifySources) for (const [relative, expected] of Object.entries(corpus.sourceFingerprints)) { const file = path.join(corpusRoot, ...relative.split("/")), end = path.join(path.dirname(file), "end.json"); if (!fs.existsSync(file) || !fs.existsSync(end)) issues.push({severity: "error", code: "missing-source", message: relative}); else if (digestBytes(Buffer.concat([fs.readFileSync(file), fs.readFileSync(end)])) !== expected) issues.push({severity: "error", code: "source-hash", message: relative}); verifiedSources += 1; }
  ...
  return {available: true, healthy: !issues.some(issue => issue.severity === "error"), summary, verifiedSources, issues: [...summary.audit.issues, ...issues]};
}

function status(): Record<string, unknown> { const summary = optional<Summary>(path.join(out, "summary.json")), state = optional<any>(path.join(out, "build-state.json")); return {available: Boolean(summary), build: state ?? null, summary: summary ? {generatedAt: summary.generatedAt, activationStatus: summary.activationStatus, corpus: summary.corpus, population: summary.population, evaluation: summary.evaluation, audit: summary.audit} : null}; }
...
function sampleMse(program: ManagerProgramV2 | undefined, samples: ManagerProgramSampleV2[]): number { return mean(samples.map(sample => ((program ? evaluateManagerProgramV2(program, "battle", sample.action, sample.features).value : 0) - sample.localValueDelta) ** 2)); }
