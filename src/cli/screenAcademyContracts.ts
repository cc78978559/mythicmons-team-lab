...
const cases = rawCases.map(value => {
  ...
  const competitiveReplayStatus = value.screenStatus === "blocked-arrears-increase" ? "blocked-arrears-increase" : cycleStatus === "retained" ? "eligible-retained-source" : cycleStatus === "unknown" ? "missing-cycle-status" : "departed-current-cycle";
...
const result = {
  ...
  summary: {cases: cases.length, blockedArrearsIncrease: cases.filter(value => value.competitiveReplayStatus === "blocked-arrears-increase").length, eligibleRetainedSources: cases.filter(value => value.competitiveReplayStatus === "eligible-retained-source").length, departedCurrentCycle: cases.filter(value => value.competitiveReplayStatus === "departed-current-cycle").length, missingCycleStatus: cases.filter(value => value.competitiveReplayStatus === "missing-cycle-status").length},
...
function report(value: typeof result): string {
  return `# Academy contract counterfactual screen\n\nCycle ${value.source.cycle}; ${value.validation.contractCount} contracts reconstructed exactly; ${value.cases.length} concession cases. ${value.summary.blockedArrearsIncrease} are blocked for increasing arrears, ${value.summary.departedCurrentCycle} departed this cycle, and ${value.summary.eligibleRetainedSources} retained sources may proceed to competitive replay. These are shadow-only contract-ledger effects, not competitive promotion evidence.\n\n| Child | Cycle | Incumbent | Candidate salary | Affected contracts | Payroll delta | Arrears delta | Academy balance delta | Replay status |\n|---|---|---|---:|---:|---:|---:|---:|---|\n${value.cases.map(item => `| ${item.childName} | ${item.cycleStatus} | ${item.incumbentStatus} | ${item.candidateSalary.toFixed(2)} | ${item.affectedChildIds.length} | ${item.payrollDelta.toFixed(2)} | ${item.arrearsDelta.toFixed(2)} | ${item.academyBalanceDelta.toFixed(2)} | ${item.competitiveReplayStatus} |`).join("\n")}\n`;
}
