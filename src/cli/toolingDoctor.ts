...
import {formalCanaryHealthIssue} from "../ai/pipelineControl";
...
const dynastyStorageBudgetBytes = numberOption("--dynasty-storage-budget-mb", 4096, 16, 102400) * 1048576, dynastyStorageGraceDays = numberOption("--dynasty-storage-grace-days", 14, 0, 3650);
...
const out = path.resolve(option("--out", path.join(root, "tooling-doctor")));
...
if (command === "status") {
  ...
  console.log(JSON.stringify({schemaVersion: 1, available: Boolean(prior), healthy: prior?.healthy ?? false, generatedAt: prior?.generatedAt ?? null, storage: prior?.storage ? {files: prior.storage.files, bytes: prior.storage.bytes, ageMs: Date.now() - Date.parse(prior.generatedAt)} : null, sourceCaches: prior?.sourceCaches ? {entries: prior.sourceCaches.entries, remainingBytes: prior.sourceCaches.remainingBytes, budgetBytes: prior.sourceCaches.budgetBytes} : null, evidenceVault: prior?.evidenceVault ?? null, readiness: prior ? {evidenceEpochReady: prior.formal?.evidenceEpochReady === true, canaryActivationReady: prior.aiPipeline?.canaryActivationReady === true} : null, pipeline: pipeline?.evaluation ? {operationalHealthy: pipeline.evaluation.operationalHealthy, pipelineCycleComplete: pipeline.evaluation.pipelineCycleComplete ?? pipeline.evaluation.researchComplete, researchMaturity: pipeline.evaluation.researchMaturity ?? null, canaryActivationReady: pipeline.evaluation.formalActivationReady === true, nextMilestones: pipeline.evaluation.nextMilestones} : null, issues: prior?.issues ?? []}, null, 2));
...
const dynastyStorage = auditAndGcDynastyStorage(league, {budgetBytes: dynastyStorageBudgetBytes, graceDays: dynastyStorageGraceDays, apply: applyGc}); atomic(path.join(out, "dynasty-storage-audit.json"), dynastyStorage);
...
else if (formal.available && !formal.evidenceEpochReady) issues.push({severity: "warning", code: "evidence-epoch-coverage-blocked", message: "League operation is healthy, but the latest season is not fully covered by the current evidence epoch"});
...
const canaryIssue = formalCanaryHealthIssue(formalValidation.eligibleDomains ?? [], formalCanary.healthy);
if (canaryIssue) issues.push(canaryIssue);
...
function formalAuditStatus(directory: string): {available: boolean; completedSeason?: number; fatalCount?: number; warningCount?: number; signatureMatches?: boolean; evidenceSignatureMatches?: boolean; evidenceEpochReady?: boolean; evidenceEpoch?: unknown; runStatus?: string | null} {
...
function formalCanaryStatus(directory: string, validationDirectory: string): {available: boolean; healthy: boolean; status?: string; domains?: number} {
  ...
  try { verifyFormalCanaryHandoff(handoff); const registryFile = path.join(directory, "adapter-registry.json"), registry = optional<FormalCanaryAdapterRegistry>(registryFile); if (fs.existsSync(registryFile) && !registry) throw new Error("Invalid formal canary adapter registry JSON"); if (registry) verifyFormalCanaryAdapterRegistry(registry); const fresh = buildFormalCanaryHandoff(freeze, summary, {seasons: handoff.control.seasons, applicationRate: handoff.control.applicationRate, maximumApplicationsPerSeason: handoff.control.maximumApplicationsPerSeason, adapterRegistry: registry}); return {available: true, healthy: fresh.sha256 === handoff.sha256, status: handoff.status, domains: handoff.domains.length}; } catch { return {available: true, healthy: false}; }
}
function aiPipelineStatus(directory: string): {available: boolean; healthy: boolean; pipelineCycleComplete?: boolean; researchMaturity?: string; canaryActivationReady?: boolean; nextStage?: string | null; warnings?: number} {
  ...
  return {available: true, healthy: evaluation.operationalHealthy === true && doctor.healthy === true && signatureMatches, pipelineCycleComplete: (evaluation.pipelineCycleComplete ?? evaluation.researchComplete) === true, researchMaturity: String(evaluation.researchMaturity ?? "legacy-unknown"), canaryActivationReady: evaluation.formalActivationReady === true, nextStage: evaluation.nextStage ?? null, warnings: Array.isArray(evaluation.warnings) ? evaluation.warnings.length : 0};
}
