...
try {
  ...
  console.error(JSON.stringify(isReadOnlyCommand(command) ? readOnlyRejected(error) : {status: "rejected", error: error instanceof Error ? error.message : String(error)}, null, 2));
  process.exitCode = 2;
}
...
function propose(): ProductionShadowSourceAuthorityManifestV1 {
  ...
  assertPhysicallySeparated(out, [{label: "source", file: input}]);
  if (source.sourceAuthority !== authority) throw new Error("Proposed production shadow source authority binding is invalid");
...
function build(): Record<string, unknown> {
  ...
  assertPhysicallySeparated(out, [{label: "source", file: input}, {label: "manifest", file: manifestFile}]);
  ...
  try {
    ...
    if (fs.existsSync(file)) {
      ...
      return {...summary("cached", out, existing, true), recoveredTemporaryFiles};
    }
    ...
    return {...summary("complete", out, persisted, true), recoveredTemporaryFiles};
...
function summaryMissing(out: string): Record<string, unknown> { return {status: "missing", available: false, healthy: false, out, sourceAuthority: null, verificationScope: "not-verified", approvedSourceRecomputed: false, activationStatus: "shadow-only", formalActivationAllowed: false, routingAllowed: false}; }
function invalidSummary(out: string, error: unknown, available: boolean): Record<string, unknown> { return {status: "invalid", available, healthy: false, out, sourceAuthority: null, verificationScope: "not-verified", approvedSourceRecomputed: false, error: error instanceof Error ? error.message : String(error), activationStatus: "shadow-only", formalActivationAllowed: false, routingAllowed: false}; }
function readOnlyRejected(error: unknown): Record<string, unknown> { return {status: "rejected", healthy: false, sourceAuthority: null, verificationScope: "not-verified", approvedSourceRecomputed: false, error: error instanceof Error ? error.message : String(error)}; }
function isReadOnlyCommand(value: string): boolean { return value === "status" || value === "doctor" || value === "inspect"; }
function summary(statusValue: string, out: string, archive: JointBattleShadowCorpusArchiveV1, approvedSourceRecomputed = false): Record<string, unknown> { return {status: statusValue, available: true, healthy: true, out, records: archive.acceptance.records, candidates: archive.corpus.metrics.candidates, informationModes: archive.informationModes, sourceAuthority: archive.corpus.sourceAuthority, sourceSha256: archive.sourceSha256, authorityManifestSha256: archive.authorityManifestSha256, sha256: archive.sha256, verificationScope: approvedSourceRecomputed ? "approved-source-recomputed" : "archive-self-check-only", approvedSourceRecomputed, activationStatus: archive.activationStatus, formalActivationAllowed: archive.formalActivationAllowed, routingAllowed: archive.routingAllowed}; }
...
function assertPhysicallySeparated(out: string, inputs: readonly {label: string; file: string}[]): void { const physicalOut = physicalPath(out); for (const input of inputs) { const physicalInput = physicalPath(input.file); if (insidePhysical(physicalInput, physicalOut)) throw new Error(`Joint shadow corpus ${input.label} must be physically outside the output root`); } }
function physicalPath(file: string): string { const absolute = path.resolve(file), tail: string[] = []; let cursor = absolute; while (!fs.existsSync(cursor)) { const parent = path.dirname(cursor); if (parent === cursor) throw new Error(`Cannot safely resolve physical path: ${file}`); tail.unshift(path.basename(cursor)); cursor = parent; } const physicalAncestor = fs.realpathSync.native(cursor); let resolved = physicalAncestor; for (const segment of tail) { if (!segment || segment === "." || segment === "..") throw new Error(`Cannot safely resolve physical path: ${file}`); resolved = path.join(resolved, segment); } return comparablePath(resolved); }
function insidePhysical(file: string, directory: string): boolean { const relative = path.relative(directory, file); return relative === "" || !relative.startsWith("..") && !path.isAbsolute(relative); }
function comparablePath(file: string): string { const normalized = path.normalize(file); return process.platform === "win32" ? normalized.toLocaleLowerCase("en-US") : normalized; }
