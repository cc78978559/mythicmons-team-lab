...
const scale30 = path.resolve(option("--scale-30", "output/development-ecology-30-reserve100-v1"));
const scale30Recovery = path.resolve(option("--scale-30-recovery", "output/development-ecology-30-reserve100-recovery-v1"));
...
const scale30Summary = read<any>(path.join(scale30, "summary.json")), scale30Baseline = read<any>(path.join(scale30, "baseline.json")), scale30RecoverySummary = read<any>(path.join(scale30Recovery, "summary.json")), scale30RecoveryIndex = scale30RecoverySummary.metrics.findIndex((metric: any) => metric.guaranteedDebt <= 1e-9);
if (scale30Summary.violations.length || scale30RecoverySummary.violations.length || scale30RecoveryIndex < 0) throw new Error("Thirty-manager scale validation did not satisfy hard invariants and bounded debt recovery");
prepareOutput();
...
const sourceFiles = [
  ...
  "docs/DEVELOPMENT_ECOLOGY_30.md",
...
const acceptance = {
  ...
  parameters: {sixAcademy: {academyGrantPool: 105, academySigningFee: 8, academyMarketOfferPercent: 115, academyEmergencySaleDiscountPercent: 35}, thirtyAcademy: {capacity: 30, academyGrantPool: 525, academyGrantLoadPercent: 100, academyPayrollReservePercent: 100, promotionSlots: 3, eliminationSlots: 3, academyMarketMaximumTransactions: 10}, marketDefault: "shadow", recoveryWindowCycles: 8},
  ...
  scale30: {status: "passed-with-recovery", configuration: scale30Baseline.configuration, observationCycles: scale30Summary.cyclesCompleted, violations: scale30Summary.violations, observationWarnings: scale30Summary.warnings, aggregate: scale30Summary.aggregate, final: scale30Summary.final, debtRecovery: {cleared: true, cyclesToClear: scale30RecoveryIndex + 1, metrics: scale30RecoverySummary.metrics.map((metric: any) => ({cycle: metric.cycle, guaranteedDebt: metric.guaranteedDebt, insolvent: metric.insolvent}))}},
  ...
  regression: {command: "npm test", status: "passed", durationSeconds: 261, note: "Full local regression completed immediately before snapshot creation."},
};
