...
const loadedPromotion = loadPromotionPackage(), promotion = loadedPromotion.payload;
const replacementIds = csvOption("--replacements");
if (!replacementIds.length) replacementIds.push(requiredOption("--replace"));
const requestedCandidateIndices = csvIntegerOption("--candidate-indices");
const candidateIndices = requestedCandidateIndices.length ? requestedCandidateIndices.map(value => value - 1) : replacementIds.length === 1 ? [integerOption("--candidate-index", 1, 1, 100) - 1] : replacementIds.map((_, index) => index);
if (candidateIndices.length !== replacementIds.length) throw new Error("--candidate-indices must contain one index per replacement");
if (new Set(replacementIds).size !== replacementIds.length) throw new Error("Promotion vacancy targets must be unique");
if (new Set(candidateIndices).size !== candidateIndices.length) throw new Error("Promotion candidates must be unique");
const assignments: PromotionAssignment[] = replacementIds.map((replacementId, index) => {
  ...
  const candidateIndex = candidateIndices[index], candidate = promotion.candidates[candidateIndex];
  ...
  return {replacementId, candidateIndex, outgoing, candidate};
});
prepareOutput();
...
const checkpointManagers: CareerMemoryCheckpoint["managers"] = major.managers.map(manager => {
  ...
  if (!assignment) return {id: manager.id, name: manager.name, baseProfile: cloneManagerProfile(manager.baseProfile), currentProfile: cloneManagerProfile(manager.currentProfile), lineage: manager.lineage, lineageHistory: manager.lineageHistory};
  ...
  return {id: manager.id, name: assignment.candidate.childName, baseProfile: cloneManagerProfile(profile), currentProfile: profile, lineage: assignment.candidate.lineage, lineageHistory: assignment.candidate.lineageHistory};
});
const checkpoint = writeCheckpoint(checkpointManagers);
const transactionRows = assignments.map(assignment => ({vacancy: assignment.replacementId, candidateIndex: assignment.candidateIndex + 1, outgoing: outgoingRecord(assignment.outgoing), incoming: incomingRecord(assignment.candidate)}));
const resets = ["major-league season record", "titles", "points", "contracts", "cash", "assets", "market"], preserves = ["all non-vacated manager personalities", "incoming development personalities", "lineages", "learned memory", "strategy programs"];
const transaction = assignments.length === 1 ? {schemaVersion: 1, authorized: true, reason, vacancy: transactionRows[0].vacancy, outgoing: transactionRows[0].outgoing, incoming: transactionRows[0].incoming, resets, preserves} : {schemaVersion: 2, authorized: true, reason, atomic: true, vacancies: replacementIds, transactions: transactionRows, resets, preserves};
writeJson(path.join(out, "promotion-transaction.json"), transaction);
runPromotedLeague(checkpoint);
verifyResult();
writeSourceTransition();
console.log(JSON.stringify({vacancies: assignments.map(value => ({vacancy: value.replacementId, outgoing: value.outgoing.name, incoming: value.candidate.childName, incomingLineage: value.candidate.lineage.lineageId})), reason, seasons, output: out}, null, 2));

function loadPromotionPackage(): {payload: PromotionPayload; sha256: string} {
...
function runPromotedLeague(checkpoint: string): void {
  ...
  const promotionSeed = assignments.map(value => value.candidate.childId).join("+");
  const env = {...process.env, V12_OUT: path.join(out, "league"), V12_SEED: `${major.seed}:promotion:${promotionSeed}`, V12_SEASONS: String(seasons), V12_MANAGER_LIMIT: String(settings.managerLimit), V12_PAIRS: String(settings.pairs), V12_POOL_SIZE: String(settings.poolSize), V12_AUCTION_LOTS: String(settings.auctionLots), V12_REGULAR_ROUNDS: String(settings.regularRounds), V12_MAX_TURNS: String(settings.maxTurns), V12_MIN_ROSTER: String(settings.minRoster ?? 6), V12_MAX_ROSTER: String(settings.maxRoster ?? 10), V12_BASE_CASH: String(settings.baseBudget ?? 40), V12_REGISTRY_SOURCE: registrySource, V12_REGISTRY_REVISION: major.registry?.revision ?? "promotion", V12_CAREER_CHECKPOINT: checkpoint, V12_ALLOW_CODE_UPGRADE: "true", V12_EVOLUTION_MODE: "punctuated", V12_EVOLUTION_POLICY: "shadow", V12_EVIDENCE_RETENTION: "compact", V12_EVIDENCE_SAMPLE_RATE: "0"};
...
function verifyResult(): void {
  ...
  for (const assignment of assignments) {
    const incoming = state.managers.find(manager => manager.id === assignment.replacementId);
    if (!incoming || incoming.lineage.lineageId !== assignment.candidate.lineage.lineageId || incoming.name !== assignment.candidate.childName) throw new Error(`Promoted manager did not occupy authorized vacancy ${assignment.replacementId}`);
  }
  const replacedIds = new Set(replacementIds);
  for (const sourceManager of major.managers.filter(manager => !replacedIds.has(manager.id))) {
...
function writeSourceTransition(): void {
  const targetRoot = path.join(out, "league"), target = read<MajorState>(path.join(targetRoot, "dynasty-state.json")), sourceBytes = fs.readFileSync(path.join(majorSource, "dynasty-state.json"));
  writeJson(path.join(targetRoot, "major-source-transition.json"), {
    schemaVersion: 1,
    type: "promotion",
    source: {root: majorSource, seed: major.seed, completedSeason: major.completedSeason, stateSha256: crypto.createHash("sha256").update(sourceBytes).digest("hex"), fingerprint: major.fingerprint, registryHash: major.registry?.hash},
    target: {root: targetRoot, seed: target.seed, completedSeason: target.completedSeason, fingerprint: target.fingerprint, registryHash: target.registry?.hash},
    promotionPackageSha256: loadedPromotion.sha256,
    transactions: transactionRows,
  });
}

function outgoingRecord(outgoing: MajorManager) { return {id: outgoing.id, name: outgoing.name, lineage: outgoing.lineage, titles: outgoing.titles, totalPoints: outgoing.totalPoints, seasons: outgoing.seasons.length}; }
function incomingRecord(candidate: PromotionCandidate) { return {childId: candidate.childId, name: candidate.childName, lineage: candidate.lineage, parentId: candidate.parentId, parentName: candidate.parentName, rightsHolderId: candidate.rightsHolderId, optionYearsRemaining: candidate.optionYearsRemaining, developmentCareer: candidate.career}; }
