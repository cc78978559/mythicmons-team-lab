...
import {FORMAL_VALIDATION_VERSION, evaluateFormalValidation, formalDomainSemanticKey, formalMechanismKey, hasExactFormalValidationIntegrity, isValidFormalMaxTurnAdjudication, selectFormalCanaryNomination, type FormalValidationCaseResult, type ValidationDirection} from "../ai/formalValidation";
...
const leagueRoot = path.resolve(option("--league", "output/official-era-03/league"));
const environments = ["modern-benchmark", "league-current"] as const;
let formalLeagueFormatId: string | null = null;
...
interface EnvironmentTeam {id: string; packed: string; tacticalProfile?: Record<string, unknown>}
interface ProgramArchive {schemaVersion: 2; activationStatus: "shadow-only"; programs: ManagerProgramV2[]}
interface FrozenHypothesis {managerId: string; rule: ManagerProgramRuleV2; discovery: {supports: number; contradictions: number; neutral: number; cases: number; independentSources: number}}
interface FrozenDomain {id: string; mechanismKey: string; semanticKey: string; target: string; expectedDirection: "better" | "worse"; adapterProtocol: typeof FORMAL_CANARY_ADAPTER_PROTOCOL; lineage: {parentSemanticKey: string | null; generation: number; intent: "novel" | "replication" | "revision"}; hypotheses: FrozenHypothesis[]; researchEvidence: {supportingManagers: number; supports: number; contradictions: number; neutral: number; cases: number; independentSources: number; discoveryFingerprints: string[]}}
interface SelectionAudit {historyPortfolioSha256: string; criteria: {minimumSupportingManagers: 2; minimumSupports: 2; maximumContradictions: 0; minimumCases: 4; minimumIndependentSources: 4; requireNovelMechanism: true}; considered: number; accepted: string[]; rejected: Array<{mechanismKey: string; reasons: string[]; priorDisposition: string | null}>; retiredMechanisms: string[]}
interface Freeze {
  ...
  environments: readonly string[]; domains: FrozenDomain[]; selection: SelectionAudit; exclusions: {stage4SourceFingerprints: string[]; retiredMechanismKeys: string[]}; benchmarkTeamSetSha256?: string; environmentTeamSetSha256: string;
  gate: {targetCases: number; minimumCasesPerEnvironment: number; targetClusters: number; minimumClustersPerEnvironment: number; maximumCasesPerCluster: number; minimumDecisiveCases: number; familywiseAlpha: number; sourceSeedLayers: number; sourceMaxTurns: number; maximumCasesPerBattle: 1; zeroUnresolvedTechnicalFailures: true}; sha256: string;
}
...
async function cycle(): Promise<Record<string, unknown>> {
  ...
  try {
    ...
    const replaceStale = args.includes("--replace-stale"), resumeFailed = args.includes("--resume"); archivedGeneration = prepareStageGeneration({rootDirectory: out, anchorName: "freeze.json", inputSignature: existingFreeze?.inputSignature ?? "unknown", retainedNames: [".formal-validation.lock"], replaceStale, validateAnchor: () => { const existingRun = optional<any>(path.join(out, "run-state.json")); if (!existingFreeze) throw new Error("Formal-validation freeze is unreadable"); if (existingRun?.status === "failed" && !resumeFailed) throw new Error("Formal-validation generation previously failed; inspect it and use --resume after repair"); validateFreeze(existingFreeze); validateFreezeBindings(existingFreeze); if (replaceStale && !Object.keys(existingFreeze.inputs).some(key => key.startsWith("benchmarkTeam:"))) throw new Error("Formal-validation freeze predates benchmark-team binding"); }});
    runState("running");
    const freeze = loadOrCreateFreeze();
    if (freeze.domains.length) installFormalRegistryRuntime(freeze);
    ...
    phase = "plan"; const plan = await loadOrBuildPlan(freeze, frontier);
...
function loadOrCreateFreeze(): Freeze {
  ...
  const stateFile = path.join(stage4, "manager-states.json.gz"), programFile = path.join(stage3, "programs.json.gz"), historyFile = path.join(out, "selection-history.json");
  if (!fs.existsSync(historyFile)) atomicJson(historyFile, loadFormalValidationPortfolio(out)); const history = read<FormalValidationPortfolio>(historyFile); verifyFormalValidationPortfolio(history);
  ...
  const programByManager = new Map(programs.map(program => [program.managerId, program])), grouped = new Map<string, Array<{managerId: string; rule: ManagerProgramRuleV2; observations: AutonomousResearchManagerState["observations"]}>>();
  for (const state of states) for (const ruleId of new Set(state.observations.map(value => value.ruleId))) {
    const observations = state.observations.filter(value => value.ruleId === ruleId), rule = programByManager.get(state.managerId)?.rules.find(value => value.id === ruleId); if (!rule) continue; const mechanismKey = formalMechanismKey(rule);
    grouped.set(mechanismKey, [...(grouped.get(mechanismKey) ?? []), {managerId: state.managerId, rule: structuredClone(rule), observations}]);
  }
  const criteria = {minimumSupportingManagers: 2 as const, minimumSupports: 2 as const, maximumContradictions: 0 as const, minimumCases: 4 as const, minimumIndependentSources: 4 as const, requireNovelMechanism: true as const}, domains: FrozenDomain[] = [], rejected: SelectionAudit["rejected"] = [];
  for (const [mechanismKey, rows] of [...grouped].sort(([left], [right]) => left.localeCompare(right))) {
    const observations = rows.flatMap(value => value.observations), supports = observations.filter(value => value.supportsHypothesis).length, contradictions = observations.filter(value => value.direction !== "neutral" && !value.supportsHypothesis).length, neutral = observations.filter(value => value.direction === "neutral").length, discoveryFingerprints = [...new Set(observations.map(value => value.sourceFingerprint))].sort(), sources = discoveryFingerprints.length, supportingRows = rows.filter(value => value.observations.some(observation => observation.supportsHypothesis) && !value.observations.some(observation => observation.direction !== "neutral" && !observation.supportsHypothesis)), familyPrior = history.feedback[mechanismKey], reasons: string[] = [], directions = new Set(rows.map(value => value.rule.effect > 0 ? "worse" : "better"));
    if (supportingRows.length < criteria.minimumSupportingManagers) reasons.push("insufficient-supporting-managers"); if (supports < criteria.minimumSupports) reasons.push("insufficient-supports"); if (contradictions > criteria.maximumContradictions) reasons.push("research-contradictions"); if (observations.length < criteria.minimumCases) reasons.push("insufficient-research-cases"); if (sources < criteria.minimumIndependentSources) reasons.push("insufficient-independent-sources"); if (directions.size !== 1) reasons.push("mixed-expected-directions");
    if (reasons.length) { rejected.push({mechanismKey, reasons, priorDisposition: familyPrior?.disposition ?? null}); continue; }
    const hypotheses: FrozenHypothesis[] = supportingRows.map(value => { const localSupports = value.observations.filter(observation => observation.supportsHypothesis).length, localContradictions = value.observations.filter(observation => observation.direction !== "neutral" && !observation.supportsHypothesis).length, localNeutral = value.observations.filter(observation => observation.direction === "neutral").length; return {managerId: value.managerId, rule: value.rule, discovery: {supports: localSupports, contradictions: localContradictions, neutral: localNeutral, cases: value.observations.length, independentSources: new Set(value.observations.map(observation => observation.sourceFingerprint)).size}}; }).sort((left, right) => left.managerId.localeCompare(right.managerId));
    const semanticKey = formalDomainSemanticKey(hypotheses), prior = history.variants[semanticKey], familyVariants = Object.values(history.variants).filter(value => value.mechanismKey === mechanismKey), parent = familyVariants.at(-1) ?? null;
    if (prior?.disposition === "rejected" || prior?.disposition === "limited-canary-eligible") reasons.push("previously-formally-decided");
    if (prior && ["inconclusive", "blocked"].includes(prior.disposition)) { const newSources = discoveryFingerprints.filter(value => !prior.discoveryFingerprints.includes(value)).length; if (prior.generations >= 2) reasons.push("replication-limit-reached"); if (newSources < 2) reasons.push("insufficient-new-replication-evidence"); }
    if (reasons.length) { rejected.push({mechanismKey, reasons, priorDisposition: prior?.disposition ?? familyPrior?.disposition ?? null}); continue; }
    const lineage = {parentSemanticKey: prior?.semanticKey ?? parent?.semanticKey ?? null, generation: prior ? prior.generations + 1 : 1, intent: prior ? "replication" as const : parent ? "revision" as const : "novel" as const};
    domains.push({id: `battle-${mechanismKey}-${digest(semanticKey).slice(0, 10)}`, mechanismKey, semanticKey, target: hypotheses[0].rule.target, expectedDirection: [...directions][0] as "better" | "worse", adapterProtocol: FORMAL_CANARY_ADAPTER_PROTOCOL, lineage, hypotheses, researchEvidence: {supportingManagers: hypotheses.length, supports, contradictions, neutral, cases: observations.length, independentSources: sources, discoveryFingerprints}});
  }
  domains.sort((left, right) => left.id.localeCompare(right.id)); const retiredMechanismKeys = Object.values(history.variants).filter(value => value.disposition === "rejected").map(value => value.semanticKey).sort(), selection: SelectionAudit = {historyPortfolioSha256: history.sha256, criteria, considered: grouped.size, accepted: domains.map(value => value.mechanismKey), rejected, retiredMechanisms: retiredMechanismKeys};
  ...
  const core = {schemaVersion: 1 as const, version: FORMAL_VALIDATION_VERSION as typeof FORMAL_VALIDATION_VERSION, authority: "validation-only-no-automatic-activation" as const, frozenAt: new Date().toISOString(), inputs, inputSignature: canonicalSha(inputs), evidenceEpoch: {policySha256: epoch.policySha256, registryHash: registry.hash, configurationPolicyVersion: LEAGUE_CONFIGURATION_POLICY_VERSION}, environments, domains, selection, exclusions: {stage4SourceFingerprints: [...discoverySources].sort(), retiredMechanismKeys}, benchmarkTeamSetSha256: benchmarkTeamSetSha256(inputs), environmentTeamSetSha256: environmentTeamSetSha256(inputs), gate: {targetCases, minimumCasesPerEnvironment: minimumPerEnvironment, targetClusters: targetCases, minimumClustersPerEnvironment: minimumPerEnvironment, maximumCasesPerCluster: 1, minimumDecisiveCases: 6, familywiseAlpha: .1, sourceSeedLayers: 5, sourceMaxTurns: 120, maximumCasesPerBattle: 1 as const, zeroUnresolvedTechnicalFailures: true as const}};
...
async function generateEnvironment(freeze: Freeze, environment: typeof environments[number], progress: (id: string) => void): Promise<SourceManifest> {
  ...
  const index = read<BenchmarkIndex>(indexFile), teams = loadEnvironmentTeams(environment), seedLayers = environment === "league-current" ? 1 : freeze.gate.sourceSeedLayers;
  ...
  for (let left = 0; left < teams.length; left += 1) for (let right = left + 1; right < teams.length; right += 1) for (let orientation = 0; orientation < 2; orientation += 1) for (let seedLayer = 1; seedLayer <= seedLayers; seedLayer += 1) jobs.push({id: `${teams[left].id}--${teams[right].id}--o${orientation + 1}--s${seedLayer}`, left, right, orientation, seedLayer});
  let cursor = 0; const worker = async () => { while (cursor < jobs.length) {
    ...
    try {
      ...
      const first = teams[job.orientation ? job.right : job.left], second = teams[job.orientation ? job.left : job.right], modernP1 = profiles[(job.left + job.right + job.seedLayer) % profiles.length], modernP2 = profiles[(job.left * 3 + job.right + job.seedLayer + 1) % profiles.length];
      const format = environment === "league-current" ? formalLeagueFormatId : index.format;
      if (!format) throw new Error("Formal league runtime format was not installed");
      const result = await runBattle({format, teamA: first.packed, teamB: second.packed, seed: `${freeze.sha256}:${environment}:${job.id}`, gameIndex: 0, outDir: parent, maxTurns: freeze.gate.sourceMaxTurns, idleTimeoutMs: 10000, wallClockTimeoutMs: 60000, ai: "search", openTeamSheets: true, traceAiDecisions: true, aiProfiles: {p1: {...(first.tacticalProfile ?? modernP1), id: `${first.id}-formal-p1`} as any, p2: {...(second.tacticalProfile ?? modernP2), id: `${second.id}-formal-p2`} as any}, evidenceContext: {registryHash: freeze.evidenceEpoch.registryHash, configurationPolicyVersion: freeze.evidenceEpoch.configurationPolicyVersion}});
...
function buildFrontier(freeze: Freeze, manifests: SourceManifest[]): FrontierRow[] {
  ...
  for (const manifest of manifests) for (const run of manifest.runs.filter(value => value.status === "complete")) {
    const game = resolveStageGame(run.game), traces = readEvidence<AiDecisionTrace[]>(game, "ai-decisions.json"), structures = teamStructuresFromReplay(read<any>(path.join(game, "replay-input.json"))), byTurn = new Map<number, Partial<Record<"p1" | "p2", AiDecisionTrace>>>();
    ...
    for (const trace of traces) {
      ...
      const opponentId = trace.playerId === "p1" ? "p2" : "p1", featureValues = pairedPositionFeatures(trace.positionSnapshot, opponent.positionSnapshot, structures[trace.playerId], structures[opponentId]), features = Object.fromEntries(POSITION_FEATURES.map((name, index) => [name, featureValues[index]]));
      ...
      Object.assign(features, {positionValue: round(predictPairedPositionValue(trace.positionSnapshot, opponent.positionSnapshot, model, structures[trace.playerId], structures[opponentId]) * 2 - 1), turnProgress: round(Math.min(1, trace.turn / 40)), scoreMargin: round(Math.tanh((Number.isFinite(margin) ? margin : 0) / 12)), candidateBreadth: round(Math.min(1, candidates.length / 12))});
      rows.push({id: `fvf-${digest([run.sourceFingerprint, trace.decisionOrdinal, trace.playerId]).slice(0, 20)}`, environment: run.environment, clusterId: `${run.environment}:${run.pair}`, game: run.game, sourceRunId: run.id, decisionOrdinal: trace.decisionOrdinal, playerId: trace.playerId, turn: trace.turn, phase: trace.turn < 14 ? "early" : trace.turn < 30 ? "mid" : "late", selected: trace.selected, selectedAction: battleActionFamily(trace.selected), features, alternatives, replaySha256: run.replaySha256!, decisionsSha256: run.decisionsSha256!, sourceFingerprint: run.sourceFingerprint!});
    }
  }
  atomicGzip(file, {schemaVersion: 1, version: FORMAL_VALIDATION_VERSION, freezeSha256: freeze.sha256, rows}); return rows;
}

async function loadOrBuildPlan(freeze: Freeze, frontier: FrontierRow[]): Promise<Plan> {
  const file = path.join(out, "plan.json"), prior = optional<Plan>(file); if (prior) { validatePlan(prior, freeze, frontier); return prior; }
  const usedBattles = new Set<string>(), clusterUsage = new Map<string, number>(), replayable = new Set<string>(), replayFailures = new Map<string, string>(), items: PlanItem[] = [], coverage: Record<string, Record<string, number>> = {};
    ...
    for (const environment of environments) {
      ...
      while (reserved.length < reserveTarget && progress) {
        ...
        for (let attempt = 0; attempt < domain.hypotheses.length && reserved.length < reserveTarget; attempt += 1) {
          const hypothesis = domain.hypotheses[hypothesisIndex++ % domain.hypotheses.length], candidates = frontier.filter(row => row.environment === environment && row.selectedAction === domain.target && !usedBattles.has(row.sourceFingerprint) && (clusterUsage.get(`${domain.id}:${row.clusterId}`) ?? 0) < freeze.gate.maximumCasesPerCluster && hypothesis.rule.predicates.every(predicate => matches(predicate, row.features))).flatMap(row => row.alternatives.filter(alternative => alternative.action !== domain.target).map(alternative => ({row, alternative}))).sort((a, b) => a.alternative.rationalCost - b.alternative.rationalCost || hashUnit(`${freeze.sha256}:${domain.id}:${hypothesis.managerId}:${a.row.id}`) - hashUnit(`${freeze.sha256}:${domain.id}:${hypothesis.managerId}:${b.row.id}`));
          let candidate: (typeof candidates)[number] | undefined;
          for (const proposed of candidates) {
            if (replayFailures.has(proposed.row.sourceFingerprint)) continue;
            if (!replayable.has(proposed.row.sourceFingerprint)) {
              const error = await replayScreenCase(proposed.row, proposed.alternative.choice);
              if (error) { replayFailures.set(proposed.row.sourceFingerprint, error); continue; }
              replayable.add(proposed.row.sourceFingerprint);
            }
            candidate = proposed; break;
          }
          if (!candidate) continue; const identity = [domain.id, hypothesis.managerId, hypothesis.rule.id, candidate.row.sourceFingerprint, candidate.row.decisionOrdinal, candidate.row.playerId, candidate.alternative.choice];
          const item: PlanCase = {id: `fvc-${digest(identity).slice(0, 20)}`, domainId: domain.id, managerId: hypothesis.managerId, ruleId: hypothesis.rule.id, environment, clusterId: candidate.row.clusterId, expectedDirection: domain.expectedDirection, game: candidate.row.game, sourceRunId: candidate.row.sourceRunId, decisionOrdinal: candidate.row.decisionOrdinal, playerId: candidate.row.playerId, turn: candidate.row.turn, phase: candidate.row.phase, incumbent: candidate.row.selected, alternative: candidate.alternative.choice, replaySha256: candidate.row.replaySha256, decisionsSha256: candidate.row.decisionsSha256, sourceFingerprint: candidate.row.sourceFingerprint};
          const clusterKey = `${domain.id}:${candidate.row.clusterId}`; reserved.push(item); usedBattles.add(candidate.row.sourceFingerprint); clusterUsage.set(clusterKey, (clusterUsage.get(clusterKey) ?? 0) + 1); progress = true;
        }
      }
      const primaryCount = Math.min(freeze.gate.minimumCasesPerEnvironment, Math.floor(reserved.length / 2));
  ...
  const replayScreen = {checked: replayable.size + replayFailures.size, eligible: replayable.size, excluded: [...replayFailures.entries()].sort(([a], [b]) => a.localeCompare(b)).map(([sourceFingerprint, error]) => ({sourceFingerprint, error}))};
  const core = {schemaVersion: 1 as const, version: FORMAL_VALIDATION_VERSION as typeof FORMAL_VALIDATION_VERSION, authority: "validation-only-no-automatic-activation" as const, freezeSha256: freeze.sha256, frontierSha256: canonicalSha(frontier), items: items.sort((a, b) => a.domainId.localeCompare(b.domainId) || a.environment.localeCompare(b.environment) || a.id.localeCompare(b.id)), coverage, replayScreen};
  const plan: Plan = {...core, sha256: canonicalSha(core)}; validatePlan(plan, freeze, frontier); atomicJson(file, plan); return plan;
}

async function replayScreenCase(row: FrontierRow, alternative: string): Promise<string | null> {
  const work = path.join(out, "work", `screen-${row.id}-${process.pid}-${crypto.randomBytes(3).toString("hex")}`);
  try {
    await child(process.execPath, [require.resolve("tsx/cli"), path.join(process.cwd(), "src/cli/counterfactualWhiteBoxBattle.ts"), "--source-game", resolveStageGame(row.game), "--out", work, "--decision-ordinal", String(row.decisionOrdinal), "--research-alternative", alternative, "--verify-source-only"]);
    return null;
...
async function executePlan(freeze: Freeze, plan: Plan, workers: number, progress: (id: string | null) => void): Promise<Results> {
  ...
  const worker = async () => { while (queue.length) { const item = queue.shift()!; progress(item.id); const cacheFile = path.join(out, "case-cache", `${item.id}.json`), cached = optional<CaseResult>(cacheFile); if (cached) { try { validateCaseResult(cached, item, freeze); cases.push(cached); continue; } catch {} }
    ...
    else { unresolvedTechnicalFailures += 1; cases.push({id: item.id, caseId: item.id, domainId: item.domainId, managerId: item.managerId, ruleId: item.ruleId, environment: item.environment, clusterId: item.clusterId, phase: item.phase, direction: "neutral", expectedDirection: item.expectedDirection, trajectoryChanged: false, winnerChanged: false, sourceVerified: false, prefixVerified: false, interventionVerified: false, sourceFingerprint: item.sourceFingerprint, technicalFailure: lastError instanceof Error ? lastError.message : String(lastError)}); }
  } };
...
async function executeCase(attributionId: string, item: PlanCase, freeze: Freeze): Promise<CaseResult> {
  ...
  try { await child(process.execPath, [require.resolve("tsx/cli"), path.join(process.cwd(), "src/cli/counterfactualWhiteBoxBattle.ts"), "--source-game", resolveStageGame(item.game), "--out", work, "--decision-ordinal", String(item.decisionOrdinal), "--research-alternative", item.alternative]); const summary = read<any>(path.join(work, "counterfactual-summary.json")), own = item.playerId === "p1" ? "Team A" : "Team B", before = winnerScore(summary.incumbent?.winner, own), after = winnerScore(summary.whitebox?.winner, own), direction: ValidationDirection = after > before ? "better" : after < before ? "worse" : "neutral";
    const result: CaseResult = {id: attributionId, caseId: item.id, domainId: item.domainId, managerId: item.managerId, ruleId: item.ruleId, environment: item.environment, clusterId: item.clusterId, phase: item.phase, direction, expectedDirection: item.expectedDirection, trajectoryChanged: summary.trajectoryChanged === true, winnerChanged: summary.winnerChanged === true, sourceVerified: summary.sourceVerified === true, prefixVerified: summary.prefixVerified === true, interventionVerified: summary.intervention?.decisionOrdinal === item.decisionOrdinal && summary.intervention?.playerId === item.playerId && summary.intervention?.expectedIncumbent === item.incumbent && summary.intervention?.selected === item.alternative, sourceFingerprint: item.sourceFingerprint};
...
function buildSummary(freeze: Freeze, manifests: SourceManifest[], frontier: FrontierRow[], plan: Plan, results: Results, compaction: {files: number; bytesBefore: number; bytesAfter: number}, elapsedMs: number, peakRssBytes: number): Record<string, any> {
  const domains = evaluateFormalValidation(freeze.domains.map(value => value.id), results.cases, freeze.environments, freeze.gate), eligible = selectFormalCanaryNomination(domains.filter(value => value.disposition === "limited-canary-eligible")), issues: Array<{severity: "error" | "warning"; code: string; message: string}> = [];
  if (results.technicalAttempts || results.unresolvedTechnicalFailures || results.fallbackSelections) issues.push({severity: "error", code: "technical-failures", message: `${results.technicalAttempts} failed attempts, ${results.fallbackSelections} fallback selections, ${results.unresolvedTechnicalFailures} unresolved cases`});
  ...
  const core = {schemaVersion: 1, version: FORMAL_VALIDATION_VERSION, generatedAt: new Date().toISOString(), authority: "validation-only-no-automatic-activation", formalValidationCompleted: !issues.some(issue => issue.severity === "error"), automaticActivationAllowed: false, limitedCanaryEligibleDomains: eligible ? [eligible.domainId] : [], inputSignature: freeze.inputSignature, freezeSha256: freeze.sha256, elapsedMs, peakRssBytes, selection: selectionProjection(freeze), sources: {environments: manifests.length, battles: manifests.reduce((sum, value) => sum + value.runs.length, 0), frontierDecisions: frontier.length, compaction: {...compaction, reclaimedBytes: compaction.bytesBefore - compaction.bytesAfter}}, experiments: {planned: plan.items.length, completed: results.cases.length - results.unresolvedTechnicalFailures, technicalAttempts: results.technicalAttempts, unresolvedTechnicalFailures: results.unresolvedTechnicalFailures, fallbackSelections: results.fallbackSelections, trajectoryChanges: results.cases.filter(value => value.trajectoryChanged).length, winnerChanges: results.cases.filter(value => value.winnerChanged).length, uniqueBattlesUsed: new Set(results.cases.map(value => value.sourceFingerprint)).size, reservedBattles: reservedSources.length, replayScreen: {checked: plan.replayScreen.checked, eligible: plan.replayScreen.eligible, excluded: plan.replayScreen.excluded.length}}, domains, audit: {healthy: !issues.some(issue => issue.severity === "error"), issues}, artifacts: {freeze: path.join(out, "freeze.json"), plan: path.join(out, "plan.json"), results: path.join(out, "results.json.gz"), report: path.join(out, "report.md")}}; return {...core, sha256: canonicalSha(core)};
}
...
function validateFreeze(value: Freeze): void {
  ...
  if (value.schemaVersion !== 1 || value.version !== FORMAL_VALIDATION_VERSION || value.authority !== "validation-only-no-automatic-activation" || canonicalSha(core) !== sha256 || canonicalSha(value.inputs) !== value.inputSignature || !Number.isFinite(Date.parse(value.frozenAt)) || JSON.stringify(value.environments) !== JSON.stringify(environments) || new Set(value.domains.map(domain => domain.id)).size !== value.domains.length || !Number.isInteger(value.gate.targetCases) || value.gate.targetCases < 1 || !Number.isInteger(value.gate.minimumCasesPerEnvironment) || value.gate.minimumCasesPerEnvironment < 1 || value.gate.minimumCasesPerEnvironment * value.environments.length < value.gate.targetCases || !Number.isInteger(value.gate.targetClusters) || value.gate.targetClusters < value.gate.targetCases || !Number.isInteger(value.gate.minimumClustersPerEnvironment) || value.gate.minimumClustersPerEnvironment < value.gate.minimumCasesPerEnvironment || !Number.isInteger(value.gate.maximumCasesPerCluster) || value.gate.maximumCasesPerCluster < 1 || !Number.isInteger(value.gate.sourceSeedLayers) || value.gate.sourceSeedLayers < 1 || value.gate.sourceSeedLayers > 8 || !Number.isInteger(value.gate.sourceMaxTurns) || value.gate.sourceMaxTurns < 20 || value.gate.sourceMaxTurns > 500 || value.gate.maximumCasesPerBattle !== 1 || value.gate.zeroUnresolvedTechnicalFailures !== true || new Set(value.exclusions.stage4SourceFingerprints).size !== value.exclusions.stage4SourceFingerprints.length || new Set(value.exclusions.retiredMechanismKeys).size !== value.exclusions.retiredMechanismKeys.length || value.selection.criteria.minimumSupportingManagers !== 2 || value.selection.criteria.minimumSupports !== 2 || value.selection.criteria.maximumContradictions !== 0 || value.selection.criteria.minimumCases !== 4 || value.selection.criteria.minimumIndependentSources !== 4 || value.selection.criteria.requireNovelMechanism !== true || JSON.stringify(value.selection.accepted) !== JSON.stringify(value.domains.map(domain => domain.mechanismKey)) || JSON.stringify(value.selection.retiredMechanisms) !== JSON.stringify(value.exclusions.retiredMechanismKeys) || value.selection.considered !== value.selection.accepted.length + value.selection.rejected.length) throw new Error("Invalid formal-validation freeze");
  for (const domain of value.domains) { const semanticKey = formalDomainSemanticKey(domain.hypotheses); if (domain.adapterProtocol !== FORMAL_CANARY_ADAPTER_PROTOCOL || domain.mechanismKey !== formalMechanismKey(domain.hypotheses[0].rule) || domain.semanticKey !== semanticKey || domain.id !== `battle-${domain.mechanismKey}-${digest(semanticKey).slice(0, 10)}` || !domain.hypotheses.length || new Set(domain.hypotheses.map(hypothesis => `${hypothesis.managerId}:${hypothesis.rule.id}`)).size !== domain.hypotheses.length || new Set(domain.hypotheses.map(hypothesis => hypothesis.managerId)).size < value.selection.criteria.minimumSupportingManagers || value.exclusions.retiredMechanismKeys.includes(domain.semanticKey) || !domain.lineage || !["novel", "replication", "revision"].includes(domain.lineage.intent) || !Number.isInteger(domain.lineage.generation) || domain.lineage.generation < 1 || domain.researchEvidence.supports < value.selection.criteria.minimumSupports || domain.researchEvidence.contradictions > value.selection.criteria.maximumContradictions || domain.researchEvidence.cases < value.selection.criteria.minimumCases || domain.researchEvidence.independentSources < value.selection.criteria.minimumIndependentSources || domain.researchEvidence.discoveryFingerprints.length !== domain.researchEvidence.independentSources || new Set(domain.researchEvidence.discoveryFingerprints).size !== domain.researchEvidence.discoveryFingerprints.length || domain.hypotheses.some(hypothesis => hypothesis.rule.target !== domain.target || formalMechanismKey(hypothesis.rule) !== domain.mechanismKey || (hypothesis.rule.effect > 0 ? "worse" : "better") !== domain.expectedDirection || hypothesis.discovery.supports < 1 || hypothesis.discovery.contradictions > 0)) throw new Error(`Invalid frozen domain: ${domain.id}`); }
}
function validateFreezeBindings(value: Freeze, archivedGeneration = false): void {
  if (archivedGeneration) { for (const [key, entry] of Object.entries(value.inputs)) if (!key || !path.isAbsolute(entry.file) || !/^[a-f0-9]{64}$/i.test(entry.sha256) || !Number.isInteger(entry.bytes) || entry.bytes < 0) throw new Error(`Invalid archived formal-validation input: ${key}`); const teamKeys = Object.keys(value.inputs).filter(key => key.startsWith("benchmarkTeam:")); if (teamKeys.length && value.benchmarkTeamSetSha256 !== benchmarkTeamSetSha256(value.inputs)) throw new Error("Archived formal-validation benchmark team set drifted"); const environmentKeys = Object.keys(value.inputs).filter(key => key.startsWith("leagueTeam:") || key === "leagueProfiles"); if (environmentKeys.length && value.environmentTeamSetSha256 !== environmentTeamSetSha256(value.inputs)) throw new Error("Archived formal-validation environment team set drifted"); return; }
  const expected = currentInputFingerprints(), baseKeys = ["stage4States", "stage4Summary", "programs", "positionModel", "benchmarkIndex", "selectionHistory"], actualKeys = Object.keys(value.inputs).sort(), expectedKeys = Object.keys(expected).sort(), hasTeamBindings = actualKeys.some(key => key.startsWith("benchmarkTeam:")), requiredKeys = hasTeamBindings ? expectedKeys : [...baseKeys].sort();
  ...
  if (value.environmentTeamSetSha256 !== environmentTeamSetSha256(expected)) throw new Error("Formal-validation environment team set drifted");
  if (!hasTeamBindings && value.benchmarkTeamSetSha256) throw new Error("Legacy formal freeze cannot claim benchmark-team binding");
  const history = read<FormalValidationPortfolio>(path.join(out, "selection-history.json")); verifyFormalValidationPortfolio(history); if (history.sha256 !== value.selection.historyPortfolioSha256) throw new Error("Formal-validation selection history drifted");
...
function validatePlan(value: Plan, freeze: Freeze, frontier?: FrontierRow[]): void {
  ...
  const excluded = value.replayScreen?.excluded ?? [], excludedSources = new Set(excluded.map(row => row.sourceFingerprint));
  if (value.schemaVersion !== 1 || value.version !== FORMAL_VALIDATION_VERSION || value.authority !== "validation-only-no-automatic-activation" || value.freezeSha256 !== freeze.sha256 || canonicalSha(core) !== sha256 || frontier && value.frontierSha256 !== canonicalSha(frontier) || new Set(value.items.map(item => item.id)).size !== value.items.length || new Set(cases.map(item => item.id)).size !== cases.length || new Set(cases.map(item => item.sourceFingerprint)).size !== cases.length || !Number.isInteger(value.replayScreen?.checked) || !Number.isInteger(value.replayScreen?.eligible) || value.replayScreen.checked !== value.replayScreen.eligible + excluded.length || value.replayScreen.eligible < cases.length || excludedSources.size !== excluded.length || excluded.some(row => !row.error) || cases.some(item => excludedSources.has(item.sourceFingerprint))) throw new Error("Invalid formal-validation plan");
  const clusterCounts = new Map<string, number>();
  for (const candidate of cases) { if (!candidate.clusterId || !candidate.clusterId.startsWith(`${candidate.environment}:`)) throw new Error(`Invalid formal-validation cluster: ${candidate.id}`); const key = `${candidate.domainId}:${candidate.clusterId}`; clusterCounts.set(key, (clusterCounts.get(key) ?? 0) + 1); }
  if ([...clusterCounts.values()].some(count => count > freeze.gate.maximumCasesPerCluster)) throw new Error("Formal-validation plan exceeds its cluster budget");
...
function validateCaseResult(result: CaseResult, item: PlanItem, freeze: Freeze): void {
  ...
  if (!candidate || result.id !== item.id || result.domainId !== candidate.domainId || result.managerId !== candidate.managerId || result.ruleId !== candidate.ruleId || result.environment !== candidate.environment || result.clusterId !== candidate.clusterId || result.phase !== candidate.phase || result.expectedDirection !== candidate.expectedDirection || result.sourceFingerprint !== candidate.sourceFingerprint || !["better", "neutral", "worse"].includes(result.direction) || typeof result.trajectoryChanged !== "boolean" || typeof result.winnerChanged !== "boolean" || result.winnerChanged && !result.trajectoryChanged || typeof result.sourceVerified !== "boolean" || typeof result.prefixVerified !== "boolean" || typeof result.interventionVerified !== "boolean" || !freeze.domains.some(domain => domain.id === result.domainId)) throw new Error(`Formal result does not match its preregistered case: ${item.id}`);
  if (result.technicalFailure) { if (hasExactFormalValidationIntegrity(result) || result.sourceVerified || result.prefixVerified || result.interventionVerified || result.direction !== "neutral" || result.trajectoryChanged || result.winnerChanged) throw new Error(`Invalid technical failure result: ${item.id}`); }
...
function validateSummary(summary: any, freeze: Freeze, manifests: SourceManifest[], plan: Plan, results: Results): void {
  ...
  const domains = evaluateFormalValidation(freeze.domains.map(domain => domain.id), results.cases, freeze.environments, freeze.gate), nomination = selectFormalCanaryNomination(domains.filter(domain => domain.disposition === "limited-canary-eligible")), eligible = nomination ? [nomination.domainId] : [], reserved = plan.items.flatMap(item => [item.sourceFingerprint, ...item.fallbacks.map(fallback => fallback.sourceFingerprint)]), expectedExperiments = {planned: plan.items.length, completed: results.cases.length - results.unresolvedTechnicalFailures, technicalAttempts: results.technicalAttempts, unresolvedTechnicalFailures: results.unresolvedTechnicalFailures, fallbackSelections: results.fallbackSelections, trajectoryChanges: results.cases.filter(result => result.trajectoryChanged).length, winnerChanges: results.cases.filter(result => result.winnerChanged).length, uniqueBattlesUsed: new Set(results.cases.map(result => result.sourceFingerprint)).size, reservedBattles: reserved.length, replayScreen: {checked: plan.replayScreen.checked, eligible: plan.replayScreen.eligible, excluded: plan.replayScreen.excluded.length}};
  const healthy = results.technicalAttempts === 0 && results.fallbackSelections === 0 && results.unresolvedTechnicalFailures === 0 && manifests.every(manifest => manifest.complete);
  if (JSON.stringify(summary.selection) !== JSON.stringify(selectionProjection(freeze)) || JSON.stringify(summary.domains) !== JSON.stringify(domains) || JSON.stringify(summary.limitedCanaryEligibleDomains) !== JSON.stringify(eligible) || JSON.stringify(summary.experiments) !== JSON.stringify(expectedExperiments) || summary.sources?.environments !== manifests.length || summary.sources?.battles !== manifests.reduce((sum, manifest) => sum + manifest.runs.length, 0) || summary.formalValidationCompleted !== healthy || summary.audit?.healthy !== healthy) throw new Error("Formal summary does not reproduce from the signed plan and results");
}
...
function report(value: any): string { return `# Stage 5 Formal Validation\n\n- Status: ${value.audit.healthy ? "healthy" : "blocked"}\n- Authority: validation only; no automatic activation\n- Mechanisms considered / accepted / retired: ${value.selection.considered} / ${value.selection.accepted} / ${value.selection.retired}\n- Prospective environments / battles: ${value.sources.environments} / ${value.sources.battles}\n- Replay screen checked / eligible / excluded: ${value.experiments.replayScreen.checked} / ${value.experiments.replayScreen.eligible} / ${value.experiments.replayScreen.excluded}\n- Exact experiments: ${value.experiments.completed}/${value.experiments.planned}\n- Trajectory / winner-changing branches: ${value.experiments.trajectoryChanges} / ${value.experiments.winnerChanges}\n- Limited-canary eligible domains: ${value.limitedCanaryEligibleDomains.length ? value.limitedCanaryEligibleDomains.join(", ") : "none"}\n\n| Domain | Cases | Clusters | Supports | Contradictions | Neutral | Adjusted p | Disposition |\n|---|---:|---:|---:|---:|---:|---:|---|\n${value.domains.map((row: any) => `| ${row.domainId} | ${row.cases} | ${row.clusters} | ${row.supports} | ${row.contradictions} | ${row.neutral} | ${row.holmAdjustedP} | ${row.disposition} |`).join("\n")}\n\nValidation used novel mechanisms and sources generated after the signed freeze in two declared environments. Inference is cluster-level, so repeated cases from one matchup cannot masquerade as independent evidence. Passing the gate grants eligibility for a separately controlled canary; it never edits or activates a manager program automatically.\n`; }
...
function loadEnvironmentTeams(environment: typeof environments[number]): EnvironmentTeam[] {
  if (environment === "modern-benchmark") { const index = read<BenchmarkIndex>(indexFile), base = path.dirname(indexFile); return index.benchmarks.map(value => ({id: value.id, packed: loadTeam(path.join(base, value.team)).packed})); }
  const season = latestLeagueSeasonDirectory(), profileRows = read<any>(path.join(season, "manager-profiles.json")).managers ?? [], profileById = new Map<string, any>(profileRows.map((value: any) => [String(value.id), value]));
  return leagueManagerIds(season).map(managerId => { const profile = profileById.get(managerId), tacticalProfile = profile?.tactics ?? profile; if (!tacticalProfile) throw new Error(`League formal environment lacks tactical profile: ${managerId}`); return {id: managerId, packed: loadTeam(path.join(season, "rosters", managerId, "roster.export.txt")).packed, tacticalProfile}; });
}

function installFormalRegistryRuntime(freeze: Freeze): void {
  const compiled = compileBoundFormalLeagueRuntime(registryRoot, freeze.evidenceEpoch.registryHash);
  installCompiledSandbox(compiled, process.cwd(), {backup: false, merge: true, replaceConflicts: false});
  formalLeagueFormatId = compiled.formatId;
}
function latestLeagueSeasonDirectory(): string {
  const seasons = fs.readdirSync(leagueRoot, {withFileTypes: true}).filter(entry => entry.isDirectory() && /^season-\d+$/.test(entry.name) && fs.existsSync(path.join(leagueRoot, entry.name, "manager-profiles.json")) && fs.existsSync(path.join(leagueRoot, entry.name, "rosters"))).map(entry => ({directory: path.join(leagueRoot, entry.name), season: Number(entry.name.slice(7))})).sort((left, right) => right.season - left.season);
  if (!seasons.length) throw new Error(`No completed league season is available for formal validation: ${leagueRoot}`); return seasons[0].directory;
}
function leagueManagerIds(season: string): string[] { return fs.readdirSync(path.join(season, "rosters"), {withFileTypes: true}).filter(entry => entry.isDirectory() && fs.existsSync(path.join(season, "rosters", entry.name, "roster.export.txt"))).map(entry => entry.name).sort(); }
function currentInputFingerprints(): Record<string, Fingerprint> {
  const index = read<BenchmarkIndex>(indexFile), base = path.dirname(indexFile), inputs: Record<string, Fingerprint> = {stage4States: fingerprint(path.join(stage4, "manager-states.json.gz")), stage4Summary: fingerprint(path.join(stage4, "summary.json")), programs: fingerprint(path.join(stage3, "programs.json.gz")), positionModel: fingerprint(path.join(positionRoot, "model.json")), benchmarkIndex: fingerprint(indexFile), selectionHistory: fingerprint(path.join(out, "selection-history.json"))};
  index.benchmarks.forEach((benchmark, position) => { inputs[`benchmarkTeam:${String(position + 1).padStart(3, "0")}:${benchmark.id}`] = fingerprint(path.join(base, benchmark.team)); });
  const season = latestLeagueSeasonDirectory(), profileFile = path.join(season, "manager-profiles.json"); inputs.leagueProfiles = fingerprint(profileFile);
  for (const managerId of leagueManagerIds(season)) inputs[`leagueTeam:${managerId}`] = fingerprint(path.join(season, "rosters", managerId, "roster.export.txt"));
  return inputs;
}
...
function environmentTeamSetSha256(inputs: Record<string, Fingerprint>): string { return canonicalSha(Object.fromEntries(Object.entries(inputs).filter(([key]) => key.startsWith("benchmarkTeam:") || key.startsWith("leagueTeam:") || key === "leagueProfiles").sort(([left], [right]) => left.localeCompare(right)))); }
function selectionProjection(freeze: Freeze): Record<string, unknown> { const reasons: Record<string, number> = {}; for (const row of freeze.selection.rejected) for (const reason of row.reasons) reasons[reason] = (reasons[reason] ?? 0) + 1; return {considered: freeze.selection.considered, accepted: freeze.selection.accepted.length, rejected: freeze.selection.rejected.length, retired: freeze.selection.retiredMechanisms.length, rejectionReasons: reasons, criteria: freeze.selection.criteria, historyPortfolioSha256: freeze.selection.historyPortfolioSha256}; }
