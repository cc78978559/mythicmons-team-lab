...
const parentLimit = integerOption("--parent-limit", Math.min(6, capacity), 1, 30), promotionSlots = integerOption("--promotion-slots", 1, 1, 5), eliminationSlots = integerOption("--elimination-slots", 1, 0, 5), regularRounds = integerOption("--regular-rounds", 1, 1, capacity - 1);
const academyGrantPool = integerOption("--academy-grant-pool", 105, 0, 100000), academyGrantLoadPercent = integerOption("--academy-grant-load-percent", 0, 0, 200), academyGrantDebtPercent = integerOption("--academy-grant-debt-percent", 0, 0, 200), academySigningFee = integerOption("--academy-signing-fee", 8, 0, 10000), academyMarketOfferPercent = integerOption("--academy-market-offer-percent", 115, 50, 200), academyEmergencySaleDiscountPercent = integerOption("--academy-emergency-sale-discount-percent", 35, 0, 90);
...
if (promotionSlots + eliminationSlots > capacity) throw new Error("Promotion and elimination slots exceed capacity");
...
for (let cycle = 1; cycle <= cycles; cycle += 1) {
  const cycleDir = path.join(out, `cycle-${String(cycle).padStart(3, "0")}`), command = [require.resolve("tsx/cli"), path.join(root, "src", "cli", "developmentLeague.ts"), "--source", source, "--out", cycleDir, "--seasons", String(seasons), "--capacity", String(capacity), "--parent-limit", String(parentLimit), "--promotion-slots", String(promotionSlots), "--elimination-slots", String(eliminationSlots), "--development-parent-percent", "50", "--max-founder-share-percent", "50", "--kinship-depth", "2", "--max-parent-similarity-percent", "90", "--academy-grant-pool", String(academyGrantPool), "--academy-grant-load-percent", String(academyGrantLoadPercent), "--academy-grant-debt-percent", String(academyGrantDebtPercent), "--academy-payroll-reserve-percent", String(academyPayrollReservePercent), "--academy-market-policy", "active", "--academy-market-consent-policy", "enforce", "--academy-market-contract-policy", "enforce", "--academy-market-offer-percent", String(academyMarketOfferPercent), "--academy-market-max-transactions", String(academyMarketMaximumTransactions), "--academy-signing-fee", String(academySigningFee), "--academy-emergency-sale-discount-percent", String(academyEmergencySaleDiscountPercent), "--regular-rounds", String(regularRounds), "--max-turns", "20"];
...
const minimumFinalFounders = Math.max(2, Math.ceil(capacity * .2)), marketDealsPerThreeCycles = Math.min(academyMarketMaximumTransactions, Math.max(1, Math.ceil(capacity / 6))), minimumMarketTransactions = Math.ceil(cycles / 3) * marketDealsPerThreeCycles;
if (final.distinctFounders < minimumFinalFounders) warnings.push(`final-founder-diversity-below-${minimumFinalFounders}`);
...
if (totalMarketTransactions < minimumMarketTransactions) warnings.push(`market-transactions-below-${minimumMarketTransactions}`);
...
const baseline = {schemaVersion: 1, name: "development-ecology-v1", createdAt: new Date().toISOString(), source: {path: source, sha256: sourceHash, version: sourceState.version, seed: sourceState.seed, completedSeason: sourceState.completedSeason, managers: sourceState.managers.length}, configuration: {cycles, seasonsPerCycle: seasons, capacity, parentLimit, promotionSlots, eliminationSlots, regularRounds, previous: initialPrevious, academyGrantPool, academyGrantLoadPercent, academyGrantDebtPercent, academyPayrollReservePercent, academySigningFee, academyMarketOfferPercent, academyEmergencySaleDiscountPercent, academyMarketMaximumTransactions, marketPolicy: "active", consentPolicy: "enforce", contractPolicy: "enforce", retention, retainedCycles: [...keepCycles].sort((a, b) => a - b)}, acceptance: {hard: ["source immutable", "population equals capacity", "unique active child ids", "nonnegative treasury", "all economic conservation errors <= 1e-6"], advisory: [`at least ${minimumFinalFounders} final founders`, "at least two final styles", "mean personality similarity <= 0.97", "average insolvency <= one third", `at least ${minimumMarketTransactions} executed market transactions`, "final average academy treasury <= 60", "final guaranteed debt equals zero"], coverage: ["count emergency-sale-eligible contracts and completed emergency sales; execution is verified by a focused deterministic test"]}};
