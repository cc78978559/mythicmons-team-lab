import {cloneTacticalMemory, type TacticalEpisode, type TacticalMemory, type TacticalPosterior} from "../../draft/tacticalMemory";
...
export function buildTacticalMemoryTrace(previous: TacticalMemory | undefined, episodes: TacticalEpisode[], season: number, decay: number, actual: TacticalMemory): TacticalMemoryTrace {
  const parameters = MEMORY_SHADOW_PARAMETERS.snapshot().values;
  const memory = cloneTacticalMemory(previous), decayEvents: TacticalMemoryTrace["decayEvents"] = [], posteriorUpdates: PosteriorUpdateTrace[] = [];
  ...
  return {version: WHITE_BOX_MEMORY_VERSION, season, parameters, decay, episodes: episodes.length, opponents: [...new Set(episodes.map(episode => episode.opponentId))].sort(), decayEvents, posteriorUpdates, decisiveEvents: episodes.reduce((sum, episode) => sum + episode.decisiveEvents.length, 0)};
}
...
function emptyOpponent(season:number){return{games:0,wins:0,losses:0,draws:0,lastSeason:season,leadCounts:{},familyImpact:{},moveImpact:{},opponentMoveCounts:{},opponentMoveCountsByFamily:{},opponentSwitches:0,observedTurns:0,episodes:[]};}
