...
export interface UnifiedBattleEvidenceAggregate {
  ...
  hypothesisId: string;
  domain: "battle";
  thresholds: {workflowSamples: 3; preliminarySamples: 10; preliminarySeeds: 5; activationSamples: number; activationSeeds: number; decisivePairs: number; maximumOneSidedP: number};
  stage: UnifiedEvidenceStage;
  conclusion: UnifiedEvidenceConclusion;
  activationEligible: boolean;
...
export type AnyUnifiedEvidenceAggregate = UnifiedEvidenceAggregate | UnifiedBattleEvidenceAggregate;
...
export function aggregateUnifiedBattleEvidence(hypothesisId: string, samples: readonly BattleCounterfactualSample[], options: {activationSamples?: number; activationSeeds?: number; decisivePairs?: number; maximumOneSidedP?: number} = {}): UnifiedBattleEvidenceAggregate {
  ...
  const battleBatch = aggregateBattleCounterfactuals(samples, {minimumSamples: activationSamples, minimumSeeds: activationSeeds, minimumDecisivePairs: decisivePairs, maximumOneSidedP});
  ...
  return {schemaVersion: 1, hypothesisId, domain: "battle", thresholds: {workflowSamples: 3, preliminarySamples: 10, preliminarySeeds: 5, activationSamples, activationSeeds, decisivePairs, maximumOneSidedP}, stage, conclusion, activationEligible, battleBatch};
}

export function unifiedEvidenceAggregateMarkdown(value: AnyUnifiedEvidenceAggregate): string {
  if ("battleBatch" in value) return `# Unified battle evidence: ${value.hypothesisId}\n\n- Stage: ${value.stage}\n- Conclusion: ${value.conclusion}\n- Formal gate: ${value.thresholds.activationSamples} samples, ${value.thresholds.activationSeeds} seeds, ${value.thresholds.decisivePairs} decisive pairs\n\n${battleCounterfactualAggregateMarkdown(value.battleBatch)}`;
