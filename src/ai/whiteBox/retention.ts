...
export function compactWhiteBoxRun(rootDirectory: string): WhiteBoxRetentionTrace {
  ...
  return {version: WHITE_BOX_RETENTION_VERSION, policy: "audit-summary", root, beforeBytes, afterBytes, removedBytes: beforeBytes - afterBytes, removedPaths, retainedPaths: ["dynasty-state.json", ".dynasty-state/**", ".season-checkpoints/**", ".runtime-bundles/**", "config-snapshots/**", "season-*/season.json", "season-*/decision-ledger.json"]};
}
