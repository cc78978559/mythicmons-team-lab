...
import {inspectHistoricalReplayPlan, type HistoricalReplayPlanInspection} from "../../draft/historicalRuntimeCheckpoint";
...
function collectBidCases(root:string,seed:string,sourceSeason:number,state:any,screenEvidence:Map<string,any>,historicalReplayFollowupSeasons:number):UnifiedEvidenceCase[]{
  ...
  const auctionMode=String(state.settings?.auctionMode??"sequential"),runtimeTransitions=historicalReplayTransitionSeasons(state.decisionRecords??[]),replayInspections=new Map<string,HistoricalReplayPlanInspection>(),bidHorizonSupported=historicalReplayFollowupSeasons<=MAX_BID_COUNTERFACTUAL_FOLLOWUP_SEASONS;
  for(let season=1;season<=sourceSeason;season+=1){
    ...
    for(const record of records.filter(entry=>entry.stage==="auction")){
      ...
      for(const item of normalized){
        ...
        const screened=screenEvidence.get(`${path.resolve(root)}|${season}|${trace.decisionId}`),baseStatus:UnifiedEvidenceStatus=auctionMode!=="sequential"?(screened?.status==="executable"?"executable":"requires-gate"):approval.recommended?"executable":"archive-only",finalSeason=season+historicalReplayFollowupSeasons,historicalTransition=runtimeTransitions.some(value=>value<=Math.min(finalSeason,sourceSeason)),replayKey=`${season}:${finalSeason}`;
        if(historicalTransition&&!replayInspections.has(replayKey))replayInspections.set(replayKey,inspectHistoricalReplayPlan(root,season,finalSeason));
        const replayInspection=historicalTransition?replayInspections.get(replayKey)!:null,historicalReady=!replayInspection||replayInspection.ready,status:UnifiedEvidenceStatus=baseStatus==="executable"&&(!bidHorizonSupported||!historicalReady)?"requires-gate":baseStatus;
        const runner:UnifiedEvidenceRunner=status==="executable"?"bid":null;
        const reasons=[...(auctionMode!=="sequential"?(screened?.status==="executable"?[]:["portfolio-solver-screen-required"]):approval.reasons),...(!bidHorizonSupported?["bid-followup-horizon-unsupported"]:[]),...(!historicalReady?["historical-runtime-transition-checkpoint-required",`historical-replay-${replayInspection&&!replayInspection.ready?replayInspection.issue:"unknown"}`]:[])];
