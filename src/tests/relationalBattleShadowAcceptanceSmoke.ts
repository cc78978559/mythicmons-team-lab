...
function record(jointSnapshot = snapshot, jointRecommendation = recommendation): UnifiedDecisionRecord { return buildUnifiedDecisionRecord({decisionId: "fixture:p1:1", domain: "battle", actor: "p1", ordinal: 1, policy: {id: "legacy-search", version: "fixture"}, information: {mode: "closed-sheet", timing: "contemporaneous", activationEligible: true, sources: ["own-private-request", "public-battle-protocol"]}, observation: {turn: 3}, options: [move, switched].map(value => ({id: value.id, score: value.values.shortValue})), selected: "move tackle", provenance: {jointRelationalSnapshot: jointSnapshot, jointBattleShadow: jointRecommendation}}); }
