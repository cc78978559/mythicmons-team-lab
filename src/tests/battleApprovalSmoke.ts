...
try{
  ...
  const evidenceEpoch={policySha256:buildEvidenceEpoch(AI_VERSION,"gen9ou").policySha256,formalActivationAllowed:true};
  fs.writeFileSync(path.join(root,"evidence-manifest.json"),JSON.stringify({evidenceEpoch,plan:{cases:[{id:hypothesis,domain:"battle",battleScopeId:scope}]}}));
