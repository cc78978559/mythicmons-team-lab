...
for (let season = 1; season <= 3; season++) for (const outcome of ["win", "loss"] as const) for (let index = 0; index < 6; index++) {
  ...
  planRows.push({season, outcome, managerId, seriesId: `series-${season}-${outcome}-${index}`, incumbentId: `${managerId}-incumbent`, candidates: [
    {id: `${managerId}-incumbent`, diagnostics: {"lineup.speedAdvantageMean": baseline, "lineup.offensivePressureFloor": baseline + 4, "lineup.speedSpread": 1}},
    {id: `${managerId}-candidate`, diagnostics: {"lineup.speedAdvantageMean": baseline + 1, "lineup.offensivePressureFloor": baseline + 5, "lineup.speedSpread": .4}},
  ]});
}
...
const gatedPreferencePlan = buildLineupHypothesisCausalPlan(planRows, registry.hypotheses[1], 6, .01, preferences, {maximumResearchPreferenceRank: 0}); assert.ok(gatedPreferencePlan.selected.every(choice => preferences.get(choice.managerId)?.researchPreferenceRank === 0), "new causal studies must honor the manager research-rank gate");
const sparsePreferencePlan = buildLineupHypothesisCausalPlan(planRows, registry.hypotheses[1], 5, .01, preferences, {allowSparseAccumulation: true, maximumResearchPreferenceRank: 0}); assert.equal(sparsePreferencePlan.causalScope, "manager-selected-accumulation"); assert.equal(sparsePreferencePlan.selected.length, 5); assert.ok(sparsePreferencePlan.selected.every(choice => preferences.get(choice.managerId)?.researchPreferenceRank === 0));
const scoped = {...registry.hypotheses[1], id: "scoped-pressure-v1", stage: "scoped-causal-candidate" as const, applicability: [{source: "delta" as const, feature: "lineup.speedSpread", maximum: -.5}], discoveryEvidence: {kind: "post-hoc-causal-heterogeneity" as const, auditSha256: "a".repeat(64), parentHypothesisId: registry.hypotheses[1].id, discoveryFinalSeason: 3}}; validateLineupHypothesisRegistry({schemaVersion: 1, activationStatus: "shadow-only", hypotheses: [scoped]}); const scopedPlan = buildLineupHypothesisCausalPlan(planRows, scoped, 6, .01); assert.equal(scopedPlan.selected.length, 6); assert.throws(() => buildLineupHypothesisCausalPlan(planRows, {...scoped, applicability: [{source: "delta" as const, feature: "lineup.speedSpread", maximum: -.7}]}, 6, .01), /Only 0\/6/);
