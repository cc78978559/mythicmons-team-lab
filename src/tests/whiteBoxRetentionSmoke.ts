...
try {
  ...
  fs.mkdirSync(path.join(root, ".season-checkpoints", "season-00"), {recursive: true});
  fs.mkdirSync(path.join(root, ".runtime-bundles", "runtime", "workspace"), {recursive: true});
  ...
  fs.writeFileSync(path.join(root, ".season-checkpoints", "season-00", "checkpoint.json"), "state replay evidence");
  fs.writeFileSync(path.join(root, ".runtime-bundles", "runtime", "workspace", "runtime.ts"), "runtime replay evidence");
  ...
  assert(fs.existsSync(path.join(root, ".season-checkpoints", "season-00", "checkpoint.json")), "Season checkpoints must survive compaction");
  assert(fs.existsSync(path.join(root, ".runtime-bundles", "runtime", "workspace", "runtime.ts")), "Historical runtimes must survive compaction");
