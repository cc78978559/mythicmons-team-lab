...
const adapterRequired = structuredClone(stages); adapterRequired[5].metrics.canaryHandoff = "adapter-required"; const adapterBlocked = evaluatePipeline(adapterRequired); assert.equal(adapterBlocked.formalActivationReady, false); assert.deepEqual(adapterBlocked.nextMilestones, ["canary-adapter"]);
const noCandidate = structuredClone(historical); noCandidate[5].metrics = {limitedCanaryEligibleDomains: []}; assert.equal(evaluatePipeline(noCandidate).researchMaturity, "iteration-required"); assert.deepEqual(evaluatePipeline(noCandidate).nextMilestones, ["current-era-evidence", "research-iteration"]);
...
assert.equal(blocked.researchMaturity, "blocked");
