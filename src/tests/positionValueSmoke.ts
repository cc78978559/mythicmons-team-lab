...
try {
  ...
  const favorable = snapshot(true), unfavorable = snapshot(false), structure = teamStructuresFromReplay(JSON.parse(fs.readFileSync(path.join(source, "cluster-0", "game-0", "replay-input.json"), "utf8"))); assert.equal(positionFeatures(favorable).length, 19); assert.equal(pairedPositionFeatures(favorable, unfavorable).length, 19); assert.equal(first.representation.structuralFeatures, 7); assert.equal(first.representation.nonConstantStructuralFeatures, 7); assert.equal(first.representation.distinctStructuralColumns, 7); assert.ok(first.representation.maximumAbsoluteStructuralCorrelation < .995); assert.ok(Object.values(structure.p1).every(value => value >= 0 && value <= 1)); assert.ok(Object.values(structure.p2).every(value => value >= 0 && value <= 1)); assert.ok(predictPairedPositionValue(favorable, unfavorable, first.model) > predictPairedPositionValue(unfavorable, favorable, first.model));
...
function packedTeam(profile: number, cluster: number): string {
  const moveSets = [["Stealth Rock", "U-turn", "Earthquake", "Toxic"], ["Rapid Spin", "Recover", "Surf", "Ice Beam"], ["Swords Dance", "Sucker Punch", "Iron Head", "Low Kick"], ["Reflect", "Light Screen", "Volt Switch", "Nuzzle"], ["Spikes", "Roost", "U-turn", "Brave Bird"], ["Nasty Plot", "Shadow Ball", "Flamethrower", "Thunderbolt"], ["Defog", "Will-O-Wisp", "Hex", "Pain Split"], ["Dragon Dance", "Extreme Speed", "Dragon Claw", "Fire Punch"], ["Leech Seed", "Synthesis", "Giga Drain", "Earth Power"], ["Tidy Up", "Bullet Seed", "Rock Blast", "Knock Off"]];
  const species = ["Landorus-Therian", "Starmie", "Corviknight", "Clefable", "Dragapult", "Kingambit", "Rotom-Wash", "Dragonite", "Gholdengo", "Great Tusk", "Rillaboom", "Volcarona"];
  return Teams.pack(Array.from({length: 6}, (_, index) => { const speciesName = species[(profile * 3 + index * (profile % 5 + 1)) % species.length], moves = moveSets[(profile + index * (profile % 4 + 1)) % moveSets.length]; return {name: `${speciesName}-${cluster}-${index}`, species: speciesName, ability: "Pressure", item: "Leftovers", moves, nature: "Serious", evs: {hp: 252, atk: 252, def: 252, spa: 252, spd: 252, spe: 252}, ivs: {hp: 31, atk: 31, def: 31, spa: 31, spd: 31, spe: 31}, gender: "", level: 100}; }));
}
