...
第一阶段统一入口已经加入：它能够跨存档扫描运营、阵容、市场及保留下来的战斗 shadow 分歧，按领域和影响去重抽样，生成配置绑定、可续跑的证据清单，并只对已有隔离重放器和门禁的案例显式运行受限实验。阵容、战斗、学习、记忆和进化仍需各自完成专用门禁与统一聚合，因此该事项尚未标记完成。详见 `docs/UNIFIED_COUNTERFACTUAL_EVIDENCE.md`。
