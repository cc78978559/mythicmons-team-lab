...
- 当前候选代码线：`agent/lineup-responsibility-graph`，正在建立 Stage 0-6 研究流水线的新发布基线；正式存档仍按内容指纹单独管理，代码合并不自动推进正式赛季。
- 正式联赛：MythicMons V12，30 名经理。
- 本地正式历史：`official-era-03` 的 S1-S24 连续；`after-s23` 七阶段已完整提交，S24 审计为 0 fatal / 0 warning。S22-S24 合计提供 2,310 场当前策略、正式上下文比赛，S24 是当前运行时边界。
- Stage 1 当前索引包含 193,301 份决策档案、18,460 场完整覆盖和 5,742 个正式兼容决策。上一轮 Stage 5 曾使用 900 场独立前瞻来源验证四个条件机制簇，两个被拒绝、两个证据不足；最新 Stage 4 没有产生干净的多经理候选，因此 Stage 5 以签名空候选结果完成，零机制取得 canary 权限。
- Manager Program V2 内核现支持 `acquire`、`configure`、`lineup`、`battle`、`research` 五个隔离域；现有正式训练和验证仍仅覆盖 `battle`，其他域必须各自建立局部价值标签、反事实回放和正式门槛后才能激活。
- 累计证据审计：16,150 场战斗、171,119 份决策档案，0 fatal；当前证据策略下的正式覆盖仍为 0。
- 正式存档位于本地 `output/`，不纳入 Git；仓库只保存代码、规则、测试与可复现命令。

## Stage 0-6 状态

- 工程健康：通过；六阶段产物、签名、依赖、恢复和全源验证均正常。
- 研究完成：通过；30 名经理形成 30 个不同的 V2 行为程序，22 名经理在自主研究中改变了后续研究意图。
- 正式激活：未通过；Stage 5 的 hazard、physical attack、special attack、switch 四域均为 inconclusive，当前没有 limited-canary 域。
- 下一里程碑：建立当前时代证据、深化组合机制研究、重新冻结正式验证，随后进入受控 canary。
- 产品边界：当前 V2 的正式研究闭环只覆盖 battle；lineup、acquire、configure 仍使用原有独立 shadow 工具。
...
- 正式历史停在 S21；`after-s21` 的 S22 周期已完成赛前审计、发展联赛、三席晋升和发展档案压缩，尚未开始 S22 正赛。周期已安全暂停，无活动进程或运行锁；下一次可从 `season` 阶段恢复，不会重复发展或晋升。
- 2026-07-26 新增统一 `league` 控制器。它以轻量状态头提供 `status/doctor/pause/resume/next/report`，合作式暂停发生在原子阶段边界，恢复参数直接来自周期清单；日常状态检查不再读取或输出完整的 147 MB dynasty 状态。
