...
npm run formal-canary -- register-adapter --id <id> --implementation <file> --targets <target,...> --reviewed
```
...
The compact Stage-2 row includes structural feature count, distinct structural columns, and held-out improvement over the untouched legacy representation. The Stage-3 row includes learned structural predicates, managers using them, and distinct structural features adopted. These are the default low-token inspection surface for the [structural information representation](INFORMATION_REPRESENTATION.md).
...
After Stage 5 completes, the controller always writes a signed formal-canary handoff. This is a control artifact, not activation: zero candidates produce `no-candidate`, and a formally eligible mechanism without a reviewed live adapter produces `adapter-required`. Only the strongest reproducible domain is nominated; handoffs containing multiple domains are invalid. A freeze declaration cannot make an adapter ready by itself. Readiness requires an optional `adapter-registry.json` beside the handoff whose signed entry names an absolute local implementation file, its current SHA-256, and the supported targets. `register-adapter` creates or updates this file locally after explicit review; planning, quick status, and both doctors recheck the registry and implementation. The CLI E2E suite exercises registration, planning, signature reconstruction, and doctor validation through production commands.
...
Historical formal feedback is ordered by the archive manifest's `archivedAt`, never by hash-derived directory names. Recovery-only archives are ignored. Any completed archive that claims formal artifacts but has an incomplete manifest, invalid signature, missing domain projection, or broken freeze/summary binding blocks Stage 5 with a readable issue instead of being silently forgotten.
...
Quick status never trusts a reported formal domain by itself. It reconstructs Stage-5 domain dispositions and experiment counters from the signed freeze, plan, and results; new summaries also carry their own canonical signature. Stage 4 and Stage 5 report decision applications, semantic trajectory changes, and winner changes separately. Canonical replay comparison ignores diagnostics and timing noise but preserves every control-relevant value.
